...
function priRename(e,id){if(e)e.stopPropagation();const card=document.getElementById('item_'+id),title=card&&card.querySelector('[data-pri-title]');if(!title)return;const old=title.textContent,input=document.createElement('input');input.className='title-edit-input';input.value=old;title.replaceWith(input);input.focus();input.select();const finish=save=>{if(!input.isConnected)return;if(save){const t=getTicks(),k='title_'+_tickStorageKey(id);t[k]=input.value.trim();saveTicks(t);}renderBriefing(window._wipData,window._wipKey);};input.onkeydown=ev=>{if(ev.key==='Enter')finish(true);if(ev.key==='Escape')finish(false);};input.onblur=()=>finish(true);}
