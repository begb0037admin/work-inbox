...
5. **Then (Drew):** build the **Teams briefing section** in `fetch_inbox.py` (`lane_b_call1.py --domain teams` already emits the normalised Teams block; needs a new briefing-section shape + a `docs/desktop-scripts` reference + LANE_B sec.6b's softer "disable NEXT run" Teams guard). Any dashboard/UI change goes through the standing screenshot + Kevin's literal "approved" gate before push -- not skipped for Lane B.
6. **Kevin's cutover go-ahead** (separate, explicit, whenever he's ready): register the live task with `-CalBackend connector` for real.
