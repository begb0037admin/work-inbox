...
Full original incident writeup: `docs/HANDOVER-LATEST-2026-09-02-teams-regression.md`. Both calendar and Teams reported `codex_failed` with only ONE `identity:"primary"` attempt logged each -- automatic Edu-primary/personal-failover never engaged, despite having been built and proven working earlier that same night in single-domain tests.

**Root cause 1 -- failover structurally could not fire.** `lane_b_call1.py`'s `PRIMARY_CODEX_HOME` resolution chain was `WI_LANE_B_CODEX_HOME` (explicit override) -> inherited `CODEX_HOME` env var -> true OS default. A `CODEX_HOME=C:\WorkInboxAI\codex-laneb` env var set during 1 Sept's now-superseded personal-only testing phase was still present (never cleared) in the live scheduled task's inherited environment on 2 Sept. That literal path is ALSO `FAILOVER_CODEX_HOME`'s own hardcoded default -- so `PRIMARY_CODEX_HOME` silently resolved to the exact same identity as `FAILOVER_CODEX_HOME`, and `fetch_domain()`'s own "nothing distinct to fail over to" branch fired every time, with no HALT, no error, nothing beyond an easily-missed log line. This also means "primary" that night was silently running as the old personal test account, not Edu -- a plausible explanation for calendar's total failure and Teams' generic timeouts (a different, less-tested account under a budget tuned for Edu).

**Root cause 2 -- Teams primary budget too tight.** The 2 Sept "further speed cut" (`PRIMARY_TIMEOUT_S=290s`, `PRIMARY_MAX_ATTEMPTS=1`) was tuned and evidenced entirely on calendar (one `list_events` call). Teams' prompt asks for up to 40 chats AND, for each chat/channel with recent activity, its 30 newest messages -- many more tool calls than calendar ever makes, and that budget was cutting it off.

**Root cause 3 (found only once #1+#2 were fixed and Teams could finally return real data for the first time) -- Teams message content was being diluted by chat metadata.** `_events_from_results()` folded `list_chats`' chat-metadata objects (id/topic/chat_type/web_url/last_message_preview) into the SAME collected list as genuine `list_chat_messages` results, and `teams_messages_to_raw()`'s field mapping (`body_preview`/`created`/`from_name`, a guessed Graph-API-style shape) never actually matched the connector's real schema (`content`/`created_at`/`author_name`/`title`) -- because no live Teams pull had ever gotten past `list_chats` before to reveal it. Confirmed live: one real successful pull returned 45 raw items, of which 40 were empty chat-metadata rows and only 5 were real messages with real content.

## The fix -- `#35`, commit `d454c7a` on `main`

1. `lane_b_call1.py`: `PRIMARY_CODEX_HOME` resolution now detects an *ambient* (not explicitly-requested via `WI_LANE_B_CODEX_HOME`) `CODEX_HOME` that collides with `FAILOVER_CODEX_HOME`'s resolved value, disregards it, and falls through to the true OS default instead -- with a loud warning and a `primary_failover_identical` flag now surfaced into every run's summary JSON (`data/lane_b/<ts>_lane_b.json`, `per_domain.<domain>.primary_failover_identical`) so this can never again pass silently. **This fix is self-contained in a file the live wrapper already pulls fresh from `main` on every single run** (see the wrapper's own refresh loop) -- it takes effect on the task's next scheduled fire automatically, no host redeploy needed.
2. `lane_b_call1.py`: Teams primary now gets its own budget (`PRIMARY_TIMEOUT_S_TEAMS`/`PRIMARY_MAX_ATTEMPTS_TEAMS`, default 360s x 2 attempts -- same as failover's already-proven budget), independent of calendar's unchanged 290s/1-attempt budget.
3. `lane_b_call1.py`: `_events_from_results()` no longer folds `list_chats`/`list_channels` into the Teams message digest (enumeration tools, not content tools); `teams_messages_to_raw()`'s field mapping corrected to the real observed schema (`content`, `created_at`, `author_name`, `title`), old guessed aliases kept as fallbacks.
4. `docs/desktop-scripts/Run Laptop Bridge Briefing.ps1`: hardened to explicitly clear `CODEX_HOME`/`WI_LANE_B_CODEX_HOME`/`WI_LANE_B_CODEX_HOME_FAILOVER` before every Lane B call, matching what every manual interactive test this week already did by hand. Defense-in-depth only -- **this file is NOT in the wrapper's own self-refresh list, so this specific hardening needs a manual redeploy of the wrapper to `C:\Users\begb0037.AD-OAK\work-inbox\Run Laptop Bridge Briefing.ps1` to take effect on the laptop.** The core regression fix (#1-3 above) does not depend on this.
5. `docs/desktop-scripts/Push-LaptopRunStatus.ps1` + the wrapper: added an optional per-domain `{status, count, served_by, primary_failover_identical}` summary (pure counts/status, no content, same whitelist discipline as the existing draftdiff stats) to the cross-machine run-status push, `data/laptop_status/briefing_status.json` -- lets the live scheduled task's real runs be checked from GitHub without RDP. **Also needs the wrapper redeployed to take effect** (same file as #4).

## Test evidence -- real, live, admin desktop's own Edu codex login (not the laptop, not a manual scheduled-task trigger)

- `py_compile` clean on all touched files; `lane_b_call1.py --selftest` 11/11; `lane_b_cal_guard.py --selftest` 7/7 (all unaffected/still green).
- Standalone harness (`test_anticollision.py`, not committed -- scratch) directly proving the collision fix against the exact incident condition: ambient `CODEX_HOME` == failover's hardcoded path -> primary now correctly resolves to the true OS default instead of colliding; an explicit `WI_LANE_B_CODEX_HOME` override to that same value is still honoured (deliberate use unaffected); a clean environment is unaffected.
- Real `--domain both` run (clean env, ts `20260903T075546Z`): **calendar status=ok served_by=primary, 94 real events** (`list_calendars`+`list_events`). Teams: primary's `list_chats` didn't fire this cycle (documented pre-existing connector flakiness, unrelated to this fix) -> correctly, for the first time, triggered a REAL failover attempt to the personal identity (proving root cause 1's fix -- this literally could not happen before today) -- the failover attempt itself then failed only because this admin desktop has no personal Lane B login directory (that only exists on the laptop), an environment gap of testing off-laptop, not a code bug. Overall run: exit 0, safely degraded, no crash, no HALT.
- Real Teams-only retry (ts `20260903T080923Z`, temporary local-only `WI_LANE_B_CODEX_HOME_FAILOVER` override pointed at the same real Edu login, for this manual test only): **status=ok served_by=primary**, `list_chats` + `list_chat_messages` both fired, completed in ~76s (well inside the new budget). Raw extraction: **5 real messages** with real sender names (Kevin Lelitte, Julie Hickman, Simon Burford), real timestamps, real chat/channel titles, and real body content (Kevin's own message linking a real SharePoint Excel file) -- confirmed via direct inspection of the raw transcript and the post-fix extraction function, before and after the metadata-pollution fix (45 items / 40 empty -> 5 items / all real).

## What's still open

1. **Live scheduled-task verification -- the actual go/no-go bar Kevin set.** Next natural fire: 12:00 BST (11:00 UTC) today, 3 Sept. Do NOT manually trigger it. Once it fires, check `data/laptop_status/briefing_status.json` (pushed automatically) for `lane_b_guard` (expect `clean`) -- the enriched `lane_b_domains` counts (item 5 above) will only appear there if the wrapper has ALSO been redeployed to the laptop (it wasn't, this session -- only `main`'s reference copy was updated). Without that redeploy, the only remotely-visible signal is the guard result -- report back what it shows regardless, and flag if per-domain counts genuinely aren't obtainable without RDP.
2. **Wrapper redeploy still pending** -- `Run Laptop Bridge Briefing.ps1`'s live copy at `C:\Users\begb0037.AD-OAK\work-inbox\Run Laptop Bridge Briefing.ps1` needs a fresh pull from `main` for items 4/5 above (env-var hardening, per-domain status visibility) to take effect. The CORE regression fix does not need this -- it's already live via auto-refresh.
3. Once the live fire is confirmed clean with real counts, this entry's own "not yet confirmed" framing should be updated to reflect that, and `docs/HANDOVER-LATEST-2026-09-02-teams-regression.md`'s "diagnosis in progress" status is now stale and can be marked resolved/superseded by this entry.
