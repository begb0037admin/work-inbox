...
- **`codex-cli 0.151.0 cold-starts SLOW on this box** -- attempt 1 of a real `codex exec` on 1 Sept took ~3m37s (13:57:52 -> 14:01:29). `lane_b_call1.py` now does a one-shot throwaway warm-up `codex exec` per process before the real calls, and the per-call timeout is 300s (`WI_LANE_B_TIMEOUT` / `WI_LANE_B_SNAP_TIMEOUT`; warm-up `WI_LANE_B_WARMUP_TIMEOUT`=360). The guard passes `WI_LANE_B_SKIP_WARMUP=1` to its `lane_b_call1` subprocess since its own PRE snapshot already warmed the box.
- **`~/.codex/config.toml` sha1 baselines are now PER-HOST** in `lane_b_call1.py` (`CONFIG_TOML_SHA1_BASELINES`): `101l-de013193` = `ba0184e864ffd081069820cc7a6f8f19acf5c845` (AD-OAK\begb0037, Edu, 0.151.0, recorded from the first real run), `desktop-mjdjm64` = `4fd8ef763bf0a8ddad9a138b6679a84fe8536f73`. A mismatch is a **WARNING only, never a HALT**; `WI_CODEX_CONFIG_SHA1` overrides; an unknown host logs a "record it" note.
