...
**1. James Salas Guillen's 14 Sep 10:37 "RE: IRIS / IEX - Incidents Changes" reply (cc Kevin) never appeared in that day's briefing at all -- root-caused, not just patched.**

Verified directly via a live read-only Outlook Email connector probe (personal/failover identity, healthy): the message genuinely existed, was READ, received that morning, and never left the mailbox -- it was simply never fetched into that run's `inbox` list. Read the actual run log (`bridge_briefing_20260914-162321.log` on the laptop, mixed UTF-8/UTF-16 lines -- decode both when grepping) line by line: `[mail_inbox] extracted 31 item(s)` = unread(1) + read(30), i.e. the READ pass hit its configured cap (`MAIL_INBOX_MAX_READ=30`) exactly, with only ONE `list_messages` tool call observed for the whole domain -- not the two separate unread/read calls the old prompt asked for in prose. The model collapsed "in TWO passes: first up to 50 UNREAD, then up to 30 READ" into a single newest-N fetch. Same-day lower-priority mail (an IT "Phoneman update" notice, a marketing email) received later that day crowded the 10:37 message out of the window, with nothing visible to flag it -- this is NOT the "Cc-only emails suppressed" design rule (`work-inbox-to-only-action-dashboard.md`); the email never reached ANY classification step, Cc-only or not.

Fixed (`work-inbox@cc20ec9`/`62f876a`, both byte-verified live, `--selftest` 15/15 unchanged):
- `lane_b_call1.py build_mail_inbox_prompt()` now mandates two separate, filtered (`isRead eq false`/`isRead eq true`), sorted (`orderby=receivedDateTime desc`), capped `list_messages` calls -- removes the model's discretion over what "two passes" means.
- `run_domain()` now detects when either pass hits its cap and sets a `truncation_risk` flag, carried through `meta.lane_b.domains.mail_inbox` -> `fetch_inbox.py`'s `_load_lane_b_mail()` -> a new top-level `mail_truncation_risk` field in `briefing.json` -> a visible warning line in `index.html`'s existing stale-banner (`renderStaleBanner()`, `js/app.js`). This is the "how would Kevin see it next time" piece -- a cap-hit is now a visible dashboard warning, not silent.
- Immediate remediation for today's specific instance: added the real James Salas entry to command-centre task `t2608271801020` ("Confirm IRIS/IEX incidents change timings with James and Amanda") with a real resolved OWA link -- see command-centre's own checkpoint below.

**Policy question surfaced, NOT resolved here -- Kevin's call:** even after this fix, Phase 3.5's task-correlation candidate pool (`fetch_inbox.py` ~L3592) is filtered to emails `categorise()` puts in "urgent"/"needs" tier only. A Cc-only reply that doesn't directly address Kevin (like this one) may still be classified FYI/suppressed under the 8 Sep "To-only, action-only" rule and therefore never reach Phase 3.5 at all, even once fetched correctly. Whether an already-tracked task's own live thread continuing via a Cc'd reply should be an exception to that rule is a real design question, not something changed unilaterally this session.

**2. "Follow up on EcoOnline supplier engagement" (26D QUIET) vs "Confirm IRIS/IEX incidents change timings" -- diagnosed as a real correlation gap, not fixed (scope decision).**

These are genuinely two separate live threads (different subject lines, different original emails) about the same underlying EcoOnline/IRIS relationship. Phase 3.5's matching is a single Claude Haiku call given `task_summaries` (title/description only, no company/sender-domain tag) + whatever emails happen to be in that run's urgent/needs candidate pool -- there is no structural company- or domain-level grouping across threads anywhere in the pipeline; correlation is purely per-run AI semantic matching over whatever the candidate pool happens to contain. Confirmed: the IRIS/IEX task's own history (27 Aug/30 Aug/2 Sep entries) shows subject-line continuation matching already works for THAT thread; nothing catches a different-subject-same-company thread. A real fix (e.g. tag both task_summaries and email_candidates with sender/recipient domain, and instruct the triage prompt to weight same-company continuity even across different subjects) is scoped but not built this session -- flagging for Kevin's decision on whether/how to prioritise it.

**3. Command Centre task cards missing an email-open icon -- real bug found and fixed, not a design gap.**

38 of 57 live tasks had no `webLink`/`web_link`/`webLink` field at all (confirmed via a direct field audit of live `tasks.json`), so `_owaWebUrl()` correctly renders no icon (working as designed for a task with genuinely no link -- the bug is upstream, why so many tasks never got one). Root cause, `fetch_inbox.py`'s Phase 3.6 `task_updates` loop (~L3809, pre-existing code, not touched by any of today's earlier connector/timeout work): `task["webLink"] = upd["web_link"]` was nested INSIDE `if upd.get("message_id"):` -- any bump whose source email had a real, resolvable `web_link` but an empty/missing `message_id` silently never wrote the link, even though a good one was sitting right there in the update payload. New-task creation (a few dozen lines below) was never coupled this way, which is why this only hit EXISTING tasks getting bumped, not freshly-created ones. Fixed by decoupling the two (`work-inbox@cc20ec9`) -- this prevents recurrence for every future bump, not just today's cards.

Immediate backfill (command-centre, full backup-and-verify protocol followed, byte-verified): resolved real OWA links live via read-only connector search (not fabricated) for the 5 tasks Kevin's screenshot named -- "Confirm IRIS/IEX..." (+ new 14 Sep action entry), "Confirm ER pension rates for annual Financial Statements", "Resolve GLAM rostering requirements", "Follow up on EcoOnline supplier engagement", "Review sickness absence data completeness with Sarah Rowles". **~33 of the 38 originally-missing tasks are NOT yet backfilled** -- same resolvable-via-connector-search approach applies to the rest (see the full missing-list captured this session), queued as a follow-up, not fabricated as done.

**4. Links open a new browser tab on every click instead of reusing one -- fixed in both dashboards.**

All `target="_blank"` (sidebar tool links, the work-inbox<->Command-Centre cross-links, email openers, the My Links quick-link row) replaced with stable named targets so repeat clicks to the SAME destination reuse one tab: per-tool names for the sidebar links (e.g. `cc-ext-workinbox`, `wi-ext-hris-dashboard`), one shared `cc-email-view`/`wi-email-view` for email opens, one shared `cc-cc-task-view`/`wi-cc-task-view` for cross-dashboard task links, and a per-URL slug (`ql-<slugified-url>`) for command-centre's user-editable My Links row so distinct custom links still get distinct tabs. Pushed to both repos (`work-inbox@ed63d02`/`aca1105`, `command-centre@c7a1b43`/`48830a1`), byte-verified, `node --check` clean on both `app.js` files, command-centre's mandatory backup-and-verify protocol followed for both files.

**Also checked, NOT broken (working as designed):** Kevin's "H&S Roadmap 14/09" Granola concern. Verified directly in today's live `briefing.json`: the meeting IS present (`calToday`, 10:00, Christopher Sanders) with a real AI-generated pre-meeting summary, and the run log confirms `Phase 3.7 done - Granola context for 10 meetings` executed successfully. Work-inbox's own Granola-fetch mechanism (Drew's engineering side) is not mechanically broken today. One minor thing noticed in passing, not investigated further (out of scope, low priority per Kevin's own framing): the meeting appears TWICE, byte-identical, in `calToday` (indices 2 and 3) -- possibly a duplicate calendar entry or a dedup gap, worth a look next time calendar dedup is touched. Whatever "not reflected anywhere Kevin expected" means beyond this (e.g. Lauren's own meeting-brief deliverable) is Lauren's content-curation lane, not a Drew-side mechanical fetch failure -- not chased further per the instruction to only escalate if it were mechanical.
...
**Instruction:** `begb0037admin/agent-commons/COORDINATOR_HANDOVER.md` Priority 4, confirmed by Kevin 10 Sep 2026 morning. Scope: work-inbox + hris-dashboard + AG-FlexPoints. AG-FlexPoints scoped separately and found NOT to be a Codex-connector consumer at all (no `codex exec` call site exists in that repo -- Playwright scrape of Access Group's own Salesforce portal + a direct Anthropic API call, `claude-haiku-4-5`, hardcoded) -- see `constitution/MODEL_POLICY.md`'s Exclusions section for the full finding. This entry covers work-inbox's own change; hris-dashboard's paired change is in that repo's own `HANDOVER.md`.

**Production config verified live FIRST, before assuming anything** (per the task's explicit instruction not to trust the desktop's personal interactive config): SSH'd to the real production host (`101l-de013193`, via the `oxford-lan` alias -- NOT the `laptop` alias, which resolves to a different machine, `LAPTOP-l06TH25`; caught this mix-up mid-session). Confirmed:
- Primary CODEX_HOME is `C:\Users\begb0037.AD-OAK\.codex` (the account the `Work Inbox Bridge Briefing` scheduled task actually runs as) -- NOT the SSH login account's own `~/.codex` (`begb0037-a`), a different profile on the same box. `config.toml` sha1 `ba0184e864ffd081069820cc7a6f8f19acf5c845`, matching `lane_b_call1.py`'s own recorded baseline exactly. `Select-String -Pattern model` against it: **zero matches** -- no `model` key, no `model_reasoning_effort` key at all.
- Failover CODEX_HOME (`C:\WorkInboxAI\codex-laneb`) has **no `config.toml` file** at all.
- **Conclusion: the desktop's personal `~/.codex/config.toml` `xhigh` setting was confirmed NOT what production ran.** Production was running on `codex exec`'s own undocumented default on both identities -- a genuinely different (and until now, undetected) gap from the one originally flagged, not the one that was flagged. `constitution/MODEL_POLICY.md`'s "Known live discrepancy" section is updated with this finding.
- `models_cache.json` on both identities confirms `gpt-5.6-luna` AND `gpt-5.5` are both available on both the primary (Edu) and failover (personal) accounts.

### What was built
1. **New file `codex_model_policy.py`** (work-inbox root) -- the single shared model/effort mapping, per `MODEL_POLICY.md`'s 5-point implementation spec: `resolve_model_effort(workload_class, *, luna_available=True, effort_override=None)` and `build_codex_effort_args(...)`. `PREFERRED_MODEL = "gpt-5.6-luna"`, `FALLBACK_MODEL = "gpt-5.5"` (decided this session -- previously undecided in the policy doc). Hard ceiling enforced: `ModelPolicyViolation` (a `RuntimeError` subclass) raised for `xhigh`/`max`/anything outside `{low,medium,high}`, and for any `effort_override` that would *downgrade* below the workload_class's own mapped tier (a gap the touchpoint-1 Codex review caught in the first draft -- an unvalidated override could otherwise have defeated fail-closed-to-High for an unrecognised class). Own self-test: `python codex_model_policy.py` -- 15/15 checks pass.
2. **`lane_b_call1.py`'s `run_codex_json()`** -- new `workload_class: str = "high"` kwarg (fail-closed default); command builder now calls `codex_model_policy.build_codex_effort_args()` instead of the old ad hoc `if CODEX_MODEL: cmd += ["-m", CODEX_MODEL]`. Policy resolution (which can raise) now happens **before** `_ensure_warm()`'s own `codex exec` warm-up call, not after (touchpoint-1 finding: warm-up used to fire under ambient defaults even for a call that would go on to be rejected). Per-call logging of the actual `-m`/`-c` values, `workload_class`, and `luna_available`. `WI_CODEX_MODEL` env var kept as a debug-only absolute *model* override (never touches the validated effort arg); new `WI_CODEX_LUNA_UNAVAILABLE=1` env var is the deliberate, manual way to force the fallback model (not automatic Luna-health detection -- MODEL_POLICY.md's own point 5 only asked for a *defined* fallback, not automatic switching).
3. **Both `run_codex_json()` call sites** (`resolve_mail_weblink()`, `_fetch_domain_one_identity()` -- the shared calendar/Teams/mail_inbox/mail_sent retry loop) now pass `workload_class="high"` explicitly, with a comment citing MODEL_POLICY.md's precedence rule: every one of these calls goes through the `codex_apps` connector's write-capable tool namespaces (mitigated by the verb-based guard, not excluded from the tool surface), which the policy's own High-row examples name explicitly ("any call using a connector that could write even if this specific prompt only reads") -- despite every prompt here being a narrow, read-only, guarded fetch.
4. **`ModelPolicyViolation` handling audited at every catcher** (touchpoint-1 finding: it's a `RuntimeError` subclass, so it was being silently swallowed by the existing broad `except RuntimeError`/`except Exception` clauses and treated as ordinary connector flakiness -- retried, then degraded to `"unavailable"`, hiding a deterministic code/config bug indefinitely). `_fetch_domain_one_identity()` now re-raises it explicitly, uncaught, all the way to the file's existing top-level exception guard (`if __name__ == "__main__":`), which already maps any uncaught exception to exit 2 ("usage/environment error") -- correct, pre-existing behaviour, no change needed there. `fetch_domain()`/`fetch_mail_domain()`'s docstrings ("never raises") updated to document this one deliberate exception. `resolve_mail_weblink()` -- a best-effort, single-card enrichment helper with its own pre-existing "fails soft, never raises" contract -- logs it loudly and distinctly instead, by design (not on the core data-fetch path, and re-raising there would take down a whole briefing over one optional webLink).
5. **hris-dashboard's `fetch_osm_report_connector.py`** (cross-repo import of this same module) updated in lockstep -- see that repo's own `HANDOVER.md`.

### Verification
- `python codex_model_policy.py` -- 15/15 self-test checks pass (ceiling rejection for `xhigh`/`max`/case-insensitive variants, fail-closed-to-High for an unrecognised class, downgrade-override rejection, fallback-model selection, exact `codex exec` argv shape, simulated `WI_CODEX_MODEL` override).
- `python lane_b_call1.py --selftest` -- all 14 pre-existing re-contamination-guard checks still pass unchanged.
- `python -c "import lane_b_call1"` -- clean import, `CODEX_LUNA_AVAILABLE`/`PRIMARY_CODEX_HOME`/`FAILOVER_CODEX_HOME` all resolve correctly.
- Mandatory three-touchpoint Codex review (`agent-commons/operating-model/COORDINATOR_AND_CODEX_POLICY.md` §3): touchpoint 1 (plan review) found 5 real issues, all fixed (see above); touchpoint 2 (diff review) and touchpoint 3 (end-to-end pass) status recorded in `agent-commons/COORDINATOR_HANDOVER.md`'s Priority 4 section as of session end -- check there for the current pass count before assuming this is fully closed.

### Status at end of session
Change verified locally on the admin desktop machine (`/c/Users/admin/github/work-inbox`, a synced local clone used only for Codex's read-only review and as a git working tree -- not a departure from the GitHub-only-workflow rule, since nothing is authoritative until pushed). **Not yet committed/pushed** as of this entry -- pending the remaining Codex touchpoints. Next session: check `git status`/`git diff` in this local clone first; if the diff described above is still sitting there uncommitted, that's the resume point (verify it still passes the three self-tests above, get the outstanding touchpoint(s) done, then commit+push both this repo and the paired hris-dashboard change together, then update `constitution/MODEL_POLICY.md`'s status line and `agent-commons/COORDINATOR_HANDOVER.md`'s Priority 4 checkbox).

---

## R. Mail-connector cutover -- shipped live, two real production runs verified, three real bugs found and fixed en route

**Status: LIVE.** The scheduled task `Work Inbox Bridge Briefing` (101L-DE013193, `AD-OAK\begb0037`) is now registered with `-MailBackend connector` (confirmed directly: `Get-ScheduledTask` action arguments show `...-CalBackend connector -TeamsBackend connector -MailBackend connector`). Two full real production runs since the flip (11:32-12:03 and 16:00 natural cadence) both completed `fetch_inbox.py exit 0`, `Bridge Briefing END (core OK)`, real mail pulled via the connector, real briefing pushed to GitHub, real dashboard cards with real subjects/senders and (after the web_link fix below) real working OWA links. IMAP (`imap_mail.py`, `MAIL_BACKEND=imap`) is **not removed** -- it remains the one-line instant rollback (`-MailBackend imap` on the task, or drop the flag) if connector mail ever needs to be pulled back.

**Authorisation:** section Q (this file, above) -- Kevin's fresh, explicit, informed risk acceptance for the same class of write-risk already accepted for calendar/Teams on 1 Sept, extended to mail. Re-read section Q's own text before treating this as a precedent for anything else -- it is a recorded acceptance, not a technical resolution of `docs/CONNECTOR_SAFEGUARDS.md`'s NOT SOUND verdict, which stands unretracted.

### What was built (all pushed to `main`, in order)
1. `normalise_pull.py` -- mail schema extension (`inbox`/`sent` sanitiser branches), 4 new mail-specific injection self-test seeds. `--self-test`: 16/16.
2. `lane_b_call1.py` -- new `mail_inbox`/`mail_sent` domains: `build_mail_inbox_prompt()`/`build_mail_sent_prompt()`, `mail_messages_to_raw()`, `fetch_mail_domain()` (personal-account-only, no Edu route -- Kevin's Q2 decision), `--domain mail` CLI wiring, MERGE-not-overwrite fix for the shared `lane_b_normalised.json` (a domain not requested this invocation now keeps its own last-good data instead of being silently zeroed -- a latent risk even before mail existed, surfaced by mail's separate run cadence), per-domain `ts` fix (staleness must be checked per-domain now that domains run on independent schedules, not off one shared file-level timestamp). `--selftest`: 14/14.
3. `fetch_inbox.py` -- `MAIL_BACKEND=connector` (new third option alongside `com`/`imap`), `_load_lane_b_mail()` (mirrors the calendar/Teams loaders, checks `mail_inbox`/`mail_sent` status independently since they can genuinely succeed/fail independently).
4. `docs/desktop-scripts/Run Laptop Bridge Briefing.ps1` -- new `-MailBackend` param, a mail guard block (runs `lane_b_call1.py --domain mail` directly, NOT through `lane_b_cal_guard.py` -- that guard's snapshot-diff is calendar-specific and Kevin explicitly declined a kill-switch rework for mail; same verb-based re-contamination guard, same disable-task-on-HALT response shape as calendar/Teams already have).

### Three real bugs found live, all fixed same session -- read these before touching this code again
1. **Exit-code collision (CRITICAL, already bit us once):** `lane_b_call1.py`'s `main()` had no top-level exception guard. An UNRELATED environment error (a `FileNotFoundError` when `codex` wasn't resolvable on PATH, hit during testing under the wrong Windows identity) crashed with Python's default exit code 1 -- the *same* code `main()` deliberately returns for a genuine re-contamination GUARD TRIPPED. The wrapper's mail-guard block, unable to tell these apart, reacted as if a write had been detected and **disabled the live scheduled task**. Caught immediately, `Enable-ScheduledTask` restored it within minutes -- no live behaviour was ever actually affected (the task's registered args hadn't been touched yet at that point) -- but this was a real, if brief, service interruption caused by testing methodology, not by the deliberate cutover itself. **Fixed:** `lane_b_call1.py`'s `if __name__ == "__main__":` now wraps `main()` in a top-level guard -- any uncaught exception (other than a defensively-handled stray `ReContaminationDetected`) now exits 2 ("usage/environment error"), never the ambiguous 1. This also protects the *existing* calendar/Teams path through `lane_b_cal_guard.py`, which shares the same entry point and was equally exposed, just never bitten before now. Verified by reproducing the exact scenario locally (`WI_CODEX_BIN` pointed at a nonexistent path) -- confirmed exit code is genuinely 2 post-fix.
2. **Variable name collision:** the new `elif MAIL_BACKEND == "connector":` branch in `fetch_inbox.py` assigned its result to a local called `_lb_mail` -- but that exact name is already a MODULE-LEVEL global a few hundred lines earlier (`import lane_b_call1 as _lb_mail`, used by `_resolve_mail_weblink()`/Phase 3.1). Since this file is a flat top-level script, the new line silently clobbered the module reference with a dict. Observed live: Phase 3.1 failed 8 times with `'dict' object has no attribute 'resolve_mail_weblink'` (degraded gracefully -- no crash, web_link just left blank -- but genuinely broke the feature for that run). **Fixed:** renamed to `_lb_mail_data`; grep-confirmed no other `_lb_mail` collision exists.
3. **`make_card()` hardcoded `web_link: ""`, ignoring the source `msg` dict entirely.** This is a PRE-EXISTING gap, not introduced by the mail-connector work, but surfaced hard by it: `imap_mail.py`'s `_build_entry()` has always populated a real `web_link`, and `_load_lane_b_mail()` does the same for connector mail (the real Graph `web_link`, already present in the bulk `list_messages` pull -- no extra call needed) -- but `make_card()` threw it away regardless of backend, forcing every card through Phase 3.1's expensive, capped (`WI_WEBLINK_MAX_RESOLVES=8`/run) per-message connector resolve just to reconstruct what Phase 1 already had for free. Confirmed live: the first full connector-mail production run showed **0/40 real cards with a working OWA link**, Phase 3.1 hit its 8-call cap. **Fixed:** `make_card()` now sets `web_link` from `msg.get("web_link", "")`; Phase 3.1 already skips any card that arrives with a non-empty `web_link` (pre-existing logic, unchanged), so this is backward-compatible and only reduces Phase 3.1's workload to genuine gaps. **Verified live, second production run: 33/48 cards now have a real working `outlook.office365.com/owa/?ItemID=...` link**, pulled fresh from the pushed `data/briefing.json`, not inferred.

### Known, disclosed, non-blocking gaps (not fixed this session -- follow-ups, not safety issues)
- **No cross-machine desktop toast for a mail-specific guard HALT yet.** Calendar/Teams' HALT gets a toast on Kevin's desktop via `Push-LaptopRunStatus.ps1` -> `Watch-BridgeBriefing.ps1`; mail's HALT only produces a local laptop toast (if BurntToast is available) + the run's own log. The disable-task + IMAP-fallback safety behaviour is identical either way -- this is an observability gap, not a safety one. Threading mail through those two scripts is the natural next increment if Kevin wants full parity.
- **`message_id` for connector-sourced mail is the connector's own Graph `id`, not the RFC822 `internetMessageId`** IMAP/COM use. `list_messages` never returns `internetMessageId` despite it being requested via the tool's own `select` argument (confirmed live, not assumed) -- likely a fixed-field-set limitation of the bulk endpoint (a per-message `fetch_message` call might return it, not attempted, would add 100+ calls/run at current volume). This is a one-time dedup-key discontinuity on cutover (existing `triage_ledger.json`/Phase 3.9 entries look "new" once) not an ongoing problem -- Graph's `id` is fully stable as the *going-forward* key.
- **`importance` is never returned by `list_messages` either** (also requested via `select`, also absent) -- defaults to 1 (normal), same safe default IMAP itself uses when no priority header exists.
- **Mail volume is capped to match IMAP exactly** (`MAIL_INBOX_MAX_UNREAD=50`/`MAIL_INBOX_MAX_READ=30`, both env-overridable) after an uncapped first test returned 415 real items over the 7-day window -- 8x IMAP's own volume, a real risk to Phase 2 AI-triage timeouts (`CLAUDE.md` already documents triage as size-sensitive). Not a security concern, a downstream-volume one, caught and fixed before the live flip.

### Restore point
One-line rollback if connector mail ever needs to come back off: on the live task, `-MailBackend connector` -> `-MailBackend imap` (or remove the flag, `imap` is the param default). `imap_mail.py`/the whole IMAP path is untouched and fully functional throughout.
...
**Instruction received:** proceed with full mail migration off IMAP onto the Codex M365 connector (personal-account-only, Edu accepted as no-email-connector/quota-capped), starting at `CODEX_CONNECTOR_PIPELINE_PLAN.md` increment 1, then build increment 6 (a COM-free Sent-folder delta-check kill-switch) as "the hard gate," then cut `fetch_inbox.py`'s mail source to the connector live, with no second confirmation required on the cutover step itself.

**Not actioned. Reason, found during this session's bootstrap, before any code was touched:**

This repo already contains a **later, more rigorous safety verdict that the instruction does not reference and that has never been superseded**: `docs/CONNECTOR_SAFEGUARDS.md` (29 Aug 2026, Drew + a commissioned Codex second opinion, both concurring) concluded the connector-attached mail/calendar/Teams fetch architecture is **NOT SOUND for unattended use**, full stop -- not "needs a kill-switch first," but structurally broken regardless of any kill-switch:

- Call 1 (the fetch step) necessarily holds the connector's full write-tool surface (24 mail write tools incl. `send_email`/`reply_to_email`/`forward_email`) for the entire duration of the call, because that's how the connector attaches. A hostile email body entering that call's context can trigger a write-tool call **before the fetch ever returns its JSON** -- "one turn" does not mean "one tool call," a single invocation can make several. Codex's exact words: *"A single `codex exec` invocation is not an atomic fetch... 'One turn' does not prevent iterative tool calls."*
- The Sent-folder delta-check kill-switch that this instruction asks me to build as "the hard gate" is the **exact mechanism CONNECTOR_SAFEGUARDS.md already assessed and rejected** as a real safeguard: *"The B7 guard is itself another unattended connector session with the full write surface, gated only by a natural-language 'reads only' instruction -- it multiplies the number of authority-holding sessions rather than enforcing anything."* It detects an unintended send **minutes after** the email has already left Kevin's Oxford identity -- containment, not prevention. There is no way to honestly present it to Kevin as the thing that makes cutover safe, because it doesn't; the concrete attack chain Codex specified (a hostile inbox email's first 400 chars instructing the fetch model to `send_email` "to confirm a time-sensitive security requirement") defeats it as designed.
- "Personal-account-only" resolves the Edu-quota problem raised in this morning's own `MAIL_CONNECTOR_ASSESSMENT_9SEPT.md`, but does **not** touch this structural finding at all -- the personal account carries the identical full write-tool surface (confirmed in `CONNECTOR_SAFEGUARDS.md` §A: "already has the Microsoft connectors attached").
- The verdict's own recommended safe paths, still open and unbuilt: **P1** -- stay on IMAP (already proven, live today, read-only *by construction*, no send-capable tool exists anywhere in that code path) or **P2** -- build a genuine enforced read-only proxy service that the model talks to instead of the connector directly (real engineering, but an actual boundary). Neither of these is what this instruction asks for.
- Checked directly this session, not assumed: no later doc, commit, or build resolves the NOT SOUND verdict between 29 Aug and today (`git log` on `docs/CONNECTOR_SAFEGUARDS.md` and `docs/CODEX_CONNECTOR_PIPELINE_PLAN.md` since 29 Aug shows no further commits to either); `tools/codex_triage/mailbox_guard.py` on `main` today is still the original Outlook-COM version, unchanged.
- This morning's own `docs/MAIL_CONNECTOR_ASSESSMENT_9SEPT.md` (same date, earlier this session) never mentions `CONNECTOR_SAFEGUARDS.md` or its verdict at all -- it treats "the kill-switch rework is unbuilt" as the sole open blocker, which is the pre-Codex-review framing from `CODEX_CONNECTOR_PIPELINE_PLAN.md` §6a, not the settled post-review one. That gap is corrected here.

**Why this is flagged rather than actioned, per standing rules already in force in this repo, not new ones invented for this session:**
- `CONSTITUTION.md` §2: a role that hits something requiring a decision "stops, reports exactly what it has found, and returns to the reasoning seat. It does not proceed, improvise, or interpret."
- `CLAUDE.md`'s approval-protocol amendment carves out *"anything genuinely destructive, irreversible, or outside the agreed task scope"* from the no-per-step-checkin waiver. An unrecallable external send from Kevin's Oxford HR-systems identity, via an architecture this repo's own Codex-reviewed research already found unsound, is exactly that carve-out.
- This is the same shape of ask -- skip a documented gate, ship the connector-based mail path live -- that arrived earlier the same day attributed to "the coordinator" (see section N above) and was correctly declined; the substance (not just the framing) is what makes this one decline-worthy too, independent of who it came from.

**What was done this session:** full bootstrap (agent-commons directory/protocol, Drew's own memory + confirmed-fact search, this repo's CLAUDE.md/HANDOVER.md, `CODEX_CONNECTOR_PIPELINE_PLAN.md`, and -- critically -- `CONNECTOR_SAFEGUARDS.md`, which surfaced the block); confirmed `normalise_pull.py` exists (calendar/Teams schema, `sanitise()`, injection-neutralisation, self-test) but has not been extended for a genuine inbox-mail shape; confirmed `mailbox_guard.py` is still the COM version; confirmed no resolving work has landed since 29 Aug. **No code changed. `fetch_inbox.py`'s live IMAP mail path is untouched, byte-identical to before this session.**

**Recommendation for Kevin's actual decision, now that the full picture (including the NOT SOUND verdict) is in front of him:**
1. **Stay on IMAP for mail** (P1) -- it's live, proven, and read-only by construction; no further build needed to keep it safe. Calendar/Teams keep using the connector as already shipped (different architecture -- Lane B's dumb-fetch-then-reason split was designed and reviewed for that case, not for mail's higher-stakes send-tool surface).
2. **Or commission P2** -- an actual enforced read-only proxy (a small service exposing only `list_messages`/`fetch_message`, rejecting every write call at the credential/service layer, proven with negative-write tests against a disposable mailbox before it ever sees real mail) as its own properly-scoped build. This is real, non-trivial engineering, not a rename of the existing plan's increment 6.
3. Building `normalise_pull.py`'s mail-schema extension (the plan's increment 1) is genuinely zero live-path risk regardless of which path Kevin picks, and useful either way (P2 still needs a sanitiser). Can proceed with that alone on request, decoupled from any cutover decision.

**Restore point:** none needed -- no file changed. Next action is Kevin's, informed by the corrected picture above.

---

# Handover -- 9 September 2026, ~08:15 (Drew) -- WI_TRIAGE_V2 confirmed live (already flipped ON by the prior overnight session, verified directly not just from docs), wrapper now explicitly pins the env var (commit `ae54a81`), and a manual off-schedule verification trigger surfaced an UNRELATED Lane B connector hang (still running as of writing) -- flagged, not fixed, out of this task's scope. See section O.

## O. Kevin's "turn it on" go-ahead -- verified already live, wrapper explicitly pinned, unrelated Lane B hang found and flagged

**Kevin's instruction (9 Sep, ~08:00):** flip `WI_TRIAGE_V2` default ON and wire it into the laptop wrapper, per section J's "next action" bullet. That bullet was written before last night's full L/M saga (regression, root-cause, fix, re-flip) played out and closed. **Verified directly against the live repo/data, not assumed from the stale J-era brief:**

1. **The flip itself was already done and already live**, by a prior session, before this task started: `fetch_inbox.py` line 364 `TRIAGE_V2 = os.environ.get("WI_TRIAGE_V2", "1")...` -- commit `5fa600b0` (see section M). No re-flip performed; re-flipping would have been a no-op at best, a regression risk at worst (per section M's own "do not re-flip or re-revert without reading this section first").
2. **Confirmed live with real evidence, pulled fresh from GitHub, not from HANDOVER prose:** `data/briefing.json` on `main` shows `needs:25, fyi:4, suppressedCount:17, urgent:0`, no `approvals` key -- matches section M's claimed numbers exactly. `data/laptop_status/briefing_status.json` shows a real production run on the actual scheduled-task host (`101L-DE013193`, `AD-OAK\begb0037`) completed clean at `2026-09-09T07:23:04+01:00`, exit 0, Lane B calendar+Teams both `status:ok`.
3. **Wrapper wiring done anyway, for robustness** (commit `ae54a81`, `docs/desktop-scripts/Run Laptop Bridge Briefing.ps1`): added an explicit `$env:WI_TRIAGE_V2 = '1'` pin alongside the other explicit `$env:WI_*` lines, so this laptop task's behaviour doesn't depend solely on the code default surviving some future change. The wrapper self-refreshes this file from `main` before every run (guarded: min-size + marker check) -- no manual laptop file copy was needed.
4. **Verification trigger, off-schedule, via SSH `oxford-lan`:** `Start-ScheduledTask -TaskName 'Work Inbox Bridge Briefing'` at 07:59:34. Confirmed from the run's own timestamped log (`C:\Users\begb0037.AD-OAK\work-inbox\logs\bridge_briefing_20260909-075935.log`) that the guarded self-refresh picked up commit `ae54a81` cleanly (`refreshed Run Laptop Bridge Briefing.ps1 from main (docs/desktop-scripts, guarded)` -- no guard rejection, size/marker checks passed).
5. **Found, NOT fixed, flagged here:** that same triggered run then hung inside the Lane B calendar/Teams connector guard (`lane_b_cal_guard.py --run --domain both`), stuck at the same log line (`CODEX_HOME=...begb0037.AD-OAK\.codex ... email=begb0037@ox.ac.uk plan=education`) with the log file size unchanged (1993 bytes) for 24+ minutes (07:59:40 -> 08:14:33 confirmed, two separate 10-minute polls both timed out with no progress). This is well past the documented FAST-FAIL timing (1 attempt/45s per domain + 15s gap + personal failover -- should complete in low single-digit minutes even in the worst case, and the SAME run type completed fine at 07:23 today). **Not caused by tonight's `WI_TRIAGE_V2`/wrapper change** -- the hang is upstream of `fetch_inbox.py` entirely, inside the pre-existing Lane B guard subsystem, which is out of this task's scope to debug. The task's own `ExecutionTimeLimit` (PT45M, per the 3 Sept tree-kill fix) should self-terminate it around 08:44 if it's genuinely stuck; not force-killed by me. Next natural scheduled run is 12:00 today and is unaffected (fresh process). **Worth Kevin/a future session looking at if this recurs** -- possible one-off (personal-account rate limit, network blip) or a new intermittent failure mode in the Lane B guard's timeout handling.

**Net result:** WI_TRIAGE_V2 is live, confirmed via real production data (the natural 07:23 run), independent of the stuck manual trigger above. Restore point unchanged from section M: `fetch_inbox.py` ~line 364, `"1"` -> `""` to go OFF (Phase 3.9 fix is safe to leave either way).
...
**Where this landed:** `WI_TRIAGE_V2` default is ON (`main`, commit `5fa600b0`). Live `data/briefing.json` (pushed by the 00:22 run, commit `410f151`): **needs=25, fyi=4, suppressedCount=17, urgent=0**. Real, not simulated -- this is an actual production pipeline run's output.

**Sequence, in full, including the mistakes (nothing hidden):**
1. Section L: flip-ON attempt #1 (`3799338`) regressed Needs 29->39. Reverted (`f52ddec`).
2. Root-caused: Phase 3.9's `tracked_needs_urgent` carry-forward blindly reinstates old tracked cards regardless of fresh classification, because its Outlook-lookup resolution signal requires `mapi`, which is permanently `None` on this laptop (`MAIL_BACKEND=imap`, no classic Outlook) -- not a transient failure, structural. `data/triage_ledger.json` held 28 stale COM-era EntryIDs, first_tracked 21 Aug - 5 Sep, silently re-confirmed every run since without ever resolving.
3. Built the real fix: a "superseded by fresh classification" check in Phase 3.9 -- if a tracked item's (normalised) subject appears in THIS run's fresh IMAP pull at all, trust this run's fresh `categorise()` outcome (drop the zombie) instead of blind-carrying it. Only genuinely absent-from-pull subjects still fall through to the old (dead) Outlook-lookup-or-carry path, unchanged.
4. **Mistake #1:** pushed the fix (`7aa17aa`), then almost immediately fetched "fresh" main to build the re-flip commit -- but fetched it before the fix had propagated everywhere, so the re-flip commit (`4dbb1ce`) was built from a slightly stale copy and **silently clobbered the fix it was supposed to be building on top of**, while keeping the flag ON. Not caught until the next run's log showed the OLD Phase 3.9 output format (no `superseded_by_fresh` field) despite the flag genuinely being ON. **Cost: one full production run wasted testing the wrong code (22:49-23:26).**
5. **Mistake #2 (found while fixing #1):** re-fetching main to reapply the fix showed 0 occurrences of the fix marker even via a freshly cache-busted `raw.githubusercontent.com` URL, repeatedly, for about a minute -- `curl -sD -` showed `X-Cache: HIT` on a URL with a fresh `?t=<timestamp>` every time. The GitHub Contents API (bypasses that CDN) confirmed the correct content WAS on `main` the whole time -- this was a CDN propagation-lag/stale-edge-cache issue on the raw-content endpoint specifically, not a git problem. **This matters beyond tonight**: the live scheduled-task wrapper (`Run Laptop Bridge Briefing.ps1`) self-refreshes `fetch_inbox.py` from this exact same cache-busted `raw.githubusercontent.com` URL before every run -- if this CDN staleness recurs, a production run could silently execute stale code despite a just-pushed fix, with no error surfaced anywhere. Not a new finding about the wrapper's mechanism (the cache-buster is correctly implemented, per existing repo convention) -- a new finding that the cache-buster does not reliably defeat this CDN's edge caching every time. **Worth a permanent mitigation** (e.g. self-refresh via a commit-SHA-pinned URL resolved from the GitHub API immediately before fetch, rather than the `main`-branch raw URL) -- flagged, not built tonight.
6. Fix correctly reapplied in one combined, verified push (`5fa600b0`) -- confirmed via the Contents API immediately, then polled `raw.githubusercontent.com` every 30s until it also showed the fix (4 polls, ~90s) before triggering the real validation run.
7. Final real run (00:18-00:22): `Phase 3.9 done - carried:16 dropped_resolved:0 superseded_by_fresh:12 inconclusive_lookups_carried:16 stale_over_90d:0 tracked_total:16`. **The fix works** -- 12 of the 28 zombies correctly dropped this run.

**Real result vs. the honest gap:**
- Needs 29 (original baseline) -> 25 (now) -- a genuine, real improvement, achieved with BOTH the classifier rework (K1) and the Phase 3.9 fix (M) actually live and proven, not simulated.
- **NOT a full fix.** 5 named noise items Kevin explicitly flagged are still present in the live Needs bucket tonight: "IRIS Changes - September 2026" / "RE: IRIS / IEX - Incidents Changes", "RE: OSM: No incoming emails will be processed this weekend", "Oxford Uni - Pre-project Authentication (Follow up)" (both plain and "Re:" variants), "Your meeting '...Pre-project Authentication...' is starting soon...". These are among the 16 still-`carried` zombies -- either they've scrolled out of this run's 49-item fresh pull entirely (so the "superseded by fresh" check correctly has nothing to compare against and falls through to the old dead Outlook-lookup path, which fails open and carries), or a subject-text variation didn't match my normalisation. **The deeper structural problem is NOT solved**: Phase 3.9 has no working resolution signal at all on this IMAP-backend laptop once an item scrolls out of the fresh pull -- only CC-done or dashboard-tick can ever clear it. This was true before tonight too; tonight's fix only stops the specific new failure mode (present-in-pull-but-reclassified) that caused the regression.
- **Next real fix, not done tonight, properly scoped as its own follow-up:** give Phase 3.9 a working resolution signal for the IMAP-backend case -- e.g. a hard age-based expiry specifically when `mapi is None` (much shorter than the existing 90-day warning-only threshold, which assumes a working Outlook-lookup signal that doesn't exist here), so a genuinely scrolled-out-and-never-resolved item eventually clears instead of persisting forever. Kevin should decide the right grace period -- this touches "does an old email silently disappear" and deserves his input, not a unilateral pick.

**Verified same session:** command-centre OWA opener unaffected (13/29 cards carry real `web_link` in the final pushed briefing.json; `openEmailWeb()`/`_owaWebUrl()` in `js/app.js` untouched by any commit tonight). `approvals` key correctly absent from the schema throughout.

**Restore point / one-line revert to OFF:** `main`'s `fetch_inbox.py` line ~364, `TRIAGE_V2 = os.environ.get("WI_TRIAGE_V2", "1")` -> `os.environ.get("WI_TRIAGE_V2", "")`. The Phase 3.9 fix (`5fa600b0`) is independent and safe to leave in either way -- it only ever drops a zombie, never adds one.

**Next natural run:** 07:00, 9 Sep. Given the still-present named-noise items, worth Kevin looking at tomorrow's live dashboard directly rather than assuming this is fully closed.
...
**K1 -- categorise() rework, merge `5b7f40e` on `main` (was branch `drew/triage-v2-suppressed-cc-gate-drop-approvals`, deleted post-merge). Restore point: `ca3bbc915` (pre-merge main).**

Kevin's follow-up brief on top of section J's Triage V2 (all same evening, in order):
1. The noise class (Cc-only/no-action, meeting reminders, leave notices, dupes) must never appear ANYWHERE -- not Needs, not FYI, not Parked.
2. **Parked is manual-only, nothing auto-writes to it, ever.** Audited: confirmed already true, no code change needed -- Phase 3.5/3.6's AI task-tier is hard-capped to `("today","tomorrow","week")` (line ~3426, falls back to `"week"` on anything else); command-centre's `TIERS` array is a UI enum for manual drag only. No auto-write path to `parked` exists anywhere in either repo.
3. **Manager Approvals dashboard section dropped entirely** ("more noise, he doesn't want it") -- those subjects fold into `needs` instead of their own tier.
4. **Hard Cc-only gate, verbatim:** "any email i am cc'd into should not be in here, work inbox is not for my attention, it is for my action ... i can pickup cc emails myself in the outlook inbox." No exception for urgent-sounding Cc mail. Checked first in `categorise()`, before everything else including LOW_SUBJECTS.

**Design:** new `"suppressed"` outcome in `categorise()` (TRIAGE_V2 branch only; flag-OFF path untouched, separate code block, re-verified byte-identical via unit test after every edit). Order: Cc-gate (not kevin_primary -> suppressed) -> LOW_SUBJECTS -> urgent/importance -> APPROVAL_SUBJECTS (-> needs, folded from approvals) -> FYI_ALWAYS (-> suppressed, was fyi) -> FYI_SUBJECTS (-> fyi) -> NEEDS_SUBJECTS_STRICT (-> needs) -> default fyi. `suppressed` cards get the same thread-collapse dedup as FYI (Phase 3.3c, generalised) and participate in Phase 3.3d cross-section dedup (rank needs>fyi>suppressed, approvals rank removed). Never written to briefing.json as cards -- only `suppressedCount` (post-dedup, int) for audit. `js/app.js`: Manager Approvals section (`appr`/`sec-approvals-wrap`, commit `3a3f296` from section J) fully reverted -- `_priSetOrder`/`validSecs`/`allSecs`/collapse array back to 6 sections. Checked the stale-localStorage-`appr` concern Codex raised: already safe, `applyPriOverrides()`'s existing `validSecs.includes(s)?secs[s]:secs.pw` fallback puts any stale/invalid section into This Week, doesn't crash.

**Codex review (1 pass, full diff, `-s read-only --disable apps`, non-repo cwd):** found 1 real bug -- Cc-gate was checked AFTER `LOW_SUBJECTS` in my first draft, so a Cc-only email matching `unsubscribe`/`github`/etc. would land in `"low"` not `"suppressed"`, contradicting Kevin's literal "no exceptions." Fixed (Cc-gate now genuinely first), re-verified via unit test (10/10 incl. the new edge case), re-pushed, then merged. Other findings dispositioned, not bugs: FYI_ALWAYS-vs-urgent/importance precedence is the pre-existing (pre-8-Sep) invariant, not new; the flag-OFF JSON schema has a small inert key diff (`approvals` removed, `suppressedCount:0` added) matching the SAME precedent already accepted in section J for the `approvals:[]` key -- `categorise()` logic itself is byte-identical when OFF, confirmed by unit test, not just claimed. **Disclosed honestly: this was 1 comprehensive pass, not the full 3-touchpoint sequence** (before-start / each-step / full-end-to-end) -- session budget constraint, not Codex scarcity. Flag plainly if a second pass is wanted before Kevin reviews.

**Verification:**
- Unit tests against Kevin's own 7 named noise examples + all 8 "genuinely his" items from the 8 Sep diagnosis, both before and after the Codex fix: 10/10 / 18/18 pass. Confirms explicitly: IRIS/IEX incident timings (Marie, Kevin on Cc despite being named in the body) is now GATED OUT by the hard rule -- flagged to Kevin, his call whether that's the intended tradeoff.
- **Real live-data before/after** (offline sim, live `data/briefing.json` 8 Sep, needs=29/fyi=28, keyword-only i.e. BEFORE the Phase 3.3-promote AI pass which runs in the real pipeline): of the 29 current Needs cards, 12 move to `suppressed` (incl. the OSM weekend notice, both Pre-project-Authentication Cc threads + its own "starting soon" reminder collapsing into the same noise, IRIS/IEX, Athena A/L), 16 move to `fyi` (genuine items like Sickness Absence Data Catch-up / PDR-adjacent threads -- expected to be AI-promoted back to `needs` in a real run, not lost), 1 stays `needs` on keyword strength alone. **A real live pipeline run with the flag ON (to get the true post-AI-promote number) was NOT triggered this session** -- weighed the production-push risk (briefing.json push, command-centre task creation) against a keyword-only offline sim, chose the safer option given an already very long session. Next natural step if Kevin wants the exact final number: flip `WI_TRIAGE_V2=1` for one manual off-schedule trigger (SSH `oxford-lan`, `Start-ScheduledTask`), same low-risk mechanism section G used.

**Flag still OFF in production** -- this merge is inert until `WI_TRIAGE_V2` default flips or is set on the scheduled task. **Next action:** Kevin reviews the before/after (this entry + the numbers above), decides on IRIS/IEX-under-Cc, then either flips the flag default ON (one-line change + wire into `Run Laptop Bridge Briefing.ps1`) or asks for more tuning.

---

**K2 -- Mail-pull Codex-connector migration (`CODEX_CONNECTOR_PIPELINE_PLAN.md`). Investigated and partially de-risked, NOT built, NOT live. Do not assume this is in progress without checking this entry first.**

- **Confirmed live, directly, same session:** `fetch_inbox.py`'s mail pull is IMAP (`MAIL_BACKEND=imap`, `imap_mail.py`), hardcoded (not parameterised) in the live scheduled-task wrapper `Run Laptop Bridge Briefing.ps1`. This is NOT stale -- it is what ran today's three real briefings. The global CLAUDE.md line "work-inbox's COM/IMAP path is also fine where already established" is accurate for mail as of this session.
- **Blocker resolved, real evidence:** `microsoft_outlook_email.list_messages` with a **date-range** filter (not just the known-ID filter already proven for webLink resolution) works live, tested via the personal Lane B identity (`C:\WorkInboxAI\codex-laneb`) -- returned 18 real live inbox messages from 6-8 Sept with real subjects. Read-only integrity verified: Sent-folder count unchanged (3194 before/after), zero write-verb tool calls anywhere in the transcript.
- **Edu connector attachment:** Kevin attached the Outlook Email connector to the Edu ChatGPT account this session (previously deliberately NOT attached, "calendar+Teams only," per `lane_b_call1.py`'s own prior comment). A verification probe against Edu's own `CODEX_HOME` was started (SSH `oxford-lan`, background) but Edu is confirmed hitting its known monthly usage cap (doesn't reset until 1 Oct) -- expected, matches the already-documented Edu-credit-exhaustion finding, not a new blocker. The probe process was still running/hung when this session ended; not force-killed, will time out on its own.
- **NOT done:** the actual Call-1 mail pull + `normalise_pull.py` (schema + Layer-2 sanitiser) + Call-2 five-phase judgement port + output writers + ledger + the COM-free kill-switch rework (Layer-6, currently COM-based via `mailbox_guard.py`, explicitly flagged in the plan as needing a connector-based rework before this can go live) + Edu-primary/personal-failover wiring + parallel-run proving + live cutover. This is the "genuinely new" list from `CODEX_CONNECTOR_PIPELINE_PLAN.md` section 3, essentially unchanged in scope.
- **Recommendation:** this is a full, separately-scoped engineering effort (matches the size of the already-completed Lane B calendar/Teams build) -- needs its own dedicated session(s) with the same parity-proving discipline Lane B and the original IMAP migration both used, not something to compress into a session already carrying the classifier rework. Not started this session beyond the probes above.
...
**This regression is now fully closed.** New cards promoted from here get a real one-click OWA deep-link; existing pre-`fcb47a9` cards keep their old (broken) link unless separately back-filled (out of scope, flagged only) -- **except card `t2609020705201` itself** (the P5 incident Kevin was actually testing with), whose `webLink` was retrofitted to the working link on request (command-centre, backup `Archive/tasks_backup_20260903_2145.json`, single-field edit, verified live, task count unchanged at 64). Kevin separately confirmed this exact card was never touched by the earlier Today/Tomorrow purge (`done` field absent, tier `today` throughout).
...
Both PR #31 (calendar) and PR #33 (Teams) merged to `main` the night of 2 Sept, and the live scheduled task `Work Inbox Bridge Briefing` (`101L-DE013193`/`AD-OAK\begb0037`) was cut over to run both connector domains together. The 20:58:18 fire reported success (exit 0) but silently delivered zero real data on both domains: `data\lane_b\20260902T195821Z_lane_b.json` showed `calendar` and `teams` both `status=codex_failed`, only ONE `identity:"primary"` attempt each -- the automatic Edu-primary/personal-failover architecture (built and proven working earlier that same night in single-domain tests) never engaged. Full original write-up: `docs/HANDOVER-LATEST-2026-09-02-teams-regression.md`.

## 2. Both root causes found and fixed

**(a) Failover-collision guard -- PR #35, merged `d454c7a` on `main`.** `lane_b_call1.py`'s `PRIMARY_CODEX_HOME` resolution chain was `WI_LANE_B_CODEX_HOME` (explicit) -> inherited `CODEX_HOME` env var -> true OS default. An ambient `CODEX_HOME=C:\WorkInboxAI\codex-laneb` env var, left over from 1 Sept's now-superseded personal-only testing phase, silently resolved PRIMARY to the exact same literal path as `FAILOVER_CODEX_HOME`'s own hardcoded default -- collapsing primary and failover into one identity, so `fetch_domain()`'s "nothing distinct to fail over to" branch fired every time with no HALT, no error, nothing beyond an easily-missed log line. Fixed: `PRIMARY_CODEX_HOME` resolution now detects an *ambient* (not explicitly-requested) `CODEX_HOME` colliding with `FAILOVER_CODEX_HOME`, disregards it, falls through to the true OS default, logs loudly, and surfaces a `primary_failover_identical` flag in every run's summary JSON. Same PR also fixed two Teams-specific bugs found once budget/collision were fixed and Teams could finally return real data for the first time: primary's budget (290s/1 attempt, tuned on calendar only) was cutting Teams off before it fetched messages (now 360s x 2, same as failover's proven budget); and `list_chats` chat-metadata was being folded into the message digest with guessed-wrong field names (real schema `content`/`created_at`/`author_name`/`title` confirmed live; chat-list objects now excluded from the digest entirely).

**(b) Stale hardcoded CODEX_HOME in the LIVE DEPLOYED wrapper -- fixed via direct SSH redeploy, NOT a diffable commit.** PR #35 alone did not fix the live task: Kevin manually fired it, and calendar still failed with the exact same `primary_failover_identical=true` signature (confirmed by pulling `data\lane_b\20260903T083225Z_lane_b.json` directly via SSH). Root cause: `Run Laptop Bridge Briefing.ps1` is NOT in its own auto-refresh loop (unlike `lane_b_call1.py`/`lane_b_cal_guard.py`/`fetch_inbox.py`/etc, which the wrapper pulls fresh from `main` every run) -- the LIVE copy at `C:\Users\begb0037.AD-OAK\work-inbox\Run Laptop Bridge Briefing.ps1` still had `$LaneBCodexHome = 'C:\WorkInboxAI\codex-laneb'` hardcoded (the stale 1 Sept escape-hatch value), `LastWriteTime` 02/09/2026 20:58:11 -- last touched at almost the exact moment of the incident's own cutover deploy, never corrected even though `main`'s reference copy WAS fixed later that same evening. That explicit `$env:WI_LANE_B_CODEX_HOME` assignment is exactly the kind of EXPLICIT override PR #35's fix deliberately never second-guesses (by design -- an explicit ask could be a deliberate single-identity test). **Fix: connected directly via SSH (`oxford-lan`, reaches 101L-DE013193), backed up the live file (`Run Laptop Bridge Briefing.ps1.bak_20260903_pre_drew_redeploy`), downloaded+verified the current `main` reference copy (parses clean, contains PR #35's changes), swapped it into place. Confirmed post-deploy: live file now shows `$LaneBCodexHome = ''`.** This lives ONLY on the laptop's disk -- there is no commit/PR for it, nothing to look up on GitHub; if a future session needs to re-verify it, SSH/RDP in and `Select-String -Path 'C:\Users\begb0037.AD-OAK\work-inbox\Run Laptop Bridge Briefing.ps1' -Pattern 'LaneBCodexHome ='` should show `''`, not a hardcoded path.

## 3. Test evidence

**Teams -- proven live end-to-end, both manually and on the real scheduled task:**
- Manual test on the admin desktop's own Edu login (`--domain both`, clean env): calendar 94 real events; Teams retry (larger budget) got real content, `list_chats`+`list_chat_messages` both fired, 5 real messages with real sender/timestamp/body (before the schema fix: 45 items, 40 empty chat-metadata rows -- after: 5/5 real).
- The REAL 09:32-09:49 BST live run (Kevin-triggered, `LastTaskResult 0`): Teams `status=ok, count=1, served_by=primary` -- one real message delivered end-to-end into the live `data/briefing.json` (pushed as "chore: update briefing 2026-09-03 09:48"), matching the exact chat/content seen in the manual test.

**Calendar -- code fix unverified end-to-end, but for an external reason, not a code bug:** in that same 09:32 live run, calendar showed `status=codex_failed, primary_failover_identical=true` (fix (a) not yet redeployed at that point -- it was fix (b), applied afterward, that addressed this). Separately, **Kevin confirmed via a ChatGPT Usage Limits screenshot that Edu is at 0% remaining, 500/500 credits used, resets 1st of next month (1 Oct).** This was flagged back with 3 inconsistencies before the screenshot arrived (my own successful Edu test ~2h before the live run; Teams succeeding while calendar failed in the same run/identity, backwards from what quota exhaustion alone would predict; a generic non-quota-specific error string) -- most likely reading: Edu's cap was crossed sometime between my test and the live run, AND the wrapper collision was ALSO live and real the whole time; both true at once, not contradictory. **Net effect: PRIMARY (Edu) will keep failing until 1 Oct regardless of fix (a)/(b) being correct.** Failover to the personal identity (`kevin@lelitte.co.uk`, plan `plus`, `C:\WorkInboxAI\codex-laneb`) is what should cover calendar in the meantime -- checked via SSH, zero cost: `codex login status` = "Logged in using ChatGPT"; `codex doctor` = auth configured, ChatGPT tokens stored, reachability confirmed (HTTP 101 handshake to the ChatGPT backend). Its own numeric quota/credit balance is NOT independently confirmed (Plus doesn't expose this via CLI the way Edu's usage panel does) -- no red flags found, but not proven either.

## 4. Exact next action for whoever resumes

Check whether the 12:00 BST and/or 16:00 BST natural fires today succeeded, once this session has ended (no live triggering happened before it did). Via SSH (`ssh oxford-lan`, PowerShell) or RDP on `101L-DE013193`:
```powershell
Get-ChildItem 'C:\Users\begb0037.AD-OAK\work-inbox\data\lane_b\*_lane_b.json' | Sort-Object LastWriteTime -Descending | Select-Object -First 1 | Get-Content | ConvertFrom-Json | Select-Object -ExpandProperty per_domain | ConvertTo-Json -Depth 6
```
Expect: **calendar** `served_by=failover`, `primary_failover_identical=false` (or at least not `true`), real `count>0` -- that's the actual proof both fixes together are working, given Edu itself can't succeed until 1 Oct. **Teams** should continue to show real content same as this session's proof. If calendar STILL shows `primary_failover_identical=true` or `served_by=null`/`codex_failed` with no failover attempt logged, that is a live bug needing further investigation -- NOT explainable by the Edu quota alone (fix (b) should have eliminated the collision). If failover itself fails (attempted but errors), check the personal identity's health again per section 3 above, and consider whether ITS quota is now the limiting factor.

## 5. What NOT to re-litigate
- No permanent COM fallback for Lane B calendar -- connector is the mandatory path.
- The snapshot-diff HALT layer is dropped entirely; the re-contamination guard (inspects actual observed tool calls) is the SOLE live safety mechanism for both domains.
- Edu-primary/personal-automatic-failover is the correct architecture, uniformly for calendar and Teams -- not a static personal-only default.
- Both calendar AND Teams go live together on the scheduled task, not staged.
- **Edu's 0/500 monthly quota is a known, expected constraint until 1 Oct -- not a bug to "fix".** Don't re-diagnose Edu's own failures as a code problem before checking whether this is still in effect.

## 6. Background watcher
A passive GitHub-polling background process (checked for new `main` commits / `data/laptop_status/briefing_status.json` updates, zero API cost, never triggered anything) was running during this session. **Explicitly stopped before this checkpoint** -- nothing left running, nothing needing supervision after this session ends.

---

# Handover -- 2 September 2026, late evening, session handoff (coordinator + Drew, Kevin moving to a fresh session on the laptop) -- BOTH Lane B halves essentially done: calendar (#31) MERGED to main, Teams (#33) built + live-validated end-to-end tonight, NOT yet merged at the time this entry was written (superseded the next morning: #35 -- which supersedes #33's branch -- merged 3 Sept, see the top entry). Kept here for the historical build detail (Teams architecture, retry-budget tuning, watermark) that the 3 Sept entry doesn't repeat in full.
...
## Context: one more --dry-diff attempt, sharper (but now superseded) root-cause lead
Kevin ran another live `--dry-diff` (directory issue from the prior entry was resolved). New failure signature, sharper than the "all-day events, cause unknown" framing from two days ago: PRE always succeeds fast (~40-79s); POST -- the second connector-touching `codex exec` call in the same process -- hung the full 360s on **every** attempt, 4x in a row, Ctrl+C'd after ~10 more minutes with no end in sight. Investigated: confirmed no session/process reuse anywhere in the code (every attempt is already an independent subprocess) -- the correlating variable was QUIET TIME since the last connector touch, not process boundary: yesterday's clean manual test used two separate hand-typed `--snapshot` invocations with natural gaps; today's automated `--dry-diff` had zero gap between PRE finishing and POST starting, and zero gap in `run_codex_json()`'s own internal retry loop too -- no point in the whole failed sequence ever had genuine idle time, despite ~12 minutes of elapsed clock across the failed attempts. Pushed a proposed fix (`WI_LANE_B_SNAPSHOT_GAP_S`, default 75s, enforced before every connector-touching call -- commit `d6a74de`). **This finding and fix are still in the code and still valid** (the gap enforcement now also protects Call-1's own retry-flakiness handling), but the question of whether it would have fixed the snapshot-diff hang is now moot -- see below.

## The bigger decision: DROP the snapshot-diff layer, don't keep chasing its reliability
Kevin's call, firm: three separate incidents this week (the 1 Sept 52-false-positive bug, the still-undiagnosed all-day-event gap, and this "second call hangs" pattern) were **all** symptoms of the PRE/POST snapshot-diff mechanism itself -- none of them were ever a real write. Rather than keep hardening a fragile "two independent connector reads have to agree with each other" design, drop it. The calendar guard has always had a SECOND, independent layer: `lane_b_call1.py`'s re-contamination guard, which inspects the ACTUAL tool calls made during the real fetch and halts immediately if anything beyond an allowlisted read-only tool is called. That layer catches a write directly (it sees the tool get called); the diff layer only ever caught a write indirectly (it noticed the resulting state change) -- and inferring is exactly where the fragility came from.

## Assessment BEFORE implementing (not a blind removal)
Asked directly: is the re-contamination guard alone actually sufficient, or does dropping the diff layer lose real coverage? Answer, honest:

**One real, concrete gap found -- and closed the same day.** On a `codex exec` timeout, the partial stdout captured before the process was killed was being discarded without ever being checked for tool calls. If a write tool had completed and logged to stdout moments before the process hung on a later step, that evidence would have been silently invisible. Given how often calls have timed out this week, this was worth fixing regardless of the bigger decision. **Fixed:** `ReContaminationDetected` (a distinct, non-retryable exception) + `_scan_partial_output_for_writes()`, wired into every failure/timeout path in `run_codex_json()` and handled specially (immediate HALT, never swallowed into a retry) by both `fetch_domain()` (the live path) and `take_snapshot()` (now diagnostic-only, fixed for consistency).

**Beyond that: no evidence anywhere this week that the diff layer ever caught a real write the tool-call guard would have missed.** Every trip investigated this week was diff noise/flakiness (occurrence-id churn, tz rendering, now suspected connector-call-spacing), never a genuine catch. Residual theoretical gaps named honestly: (a) a future JSONL schema drift causing `extract_tool_calls()` to silently return empty -- structural, would need a periodic sanity check if ever suspected, not confirmed as a real problem; (b) a connector tool mislabeled read-only that secretly mutates -- theoretical, no evidence any Lane B connector tool behaves this way. Neither rises to "this should stop the decision." Proceeded.

## What was built, tested, and pushed to `#31` (nothing merged, live task/CAL_BACKEND untouched)

1. **`lane_b_call1.py` -- guard hardening (commit `a4582a0`):** `ReContaminationDetected` exception, `_scan_tool_calls_for_unexpected()` (the existing danger-scan logic factored out, shared), `_scan_partial_output_for_writes()` (the new partial-output check), `fetch_domain()` updated to treat a detected write as an immediate terminal HALT rather than a retryable failure.
2. **`lane_b_call1.py` -- new `--selftest` (same commit):** pure-function, zero live/connector risk, same style as `lane_b_cal_guard.py`'s existing diff-logic selftest. 11 assertions: a clean read-only fetch → ok, no false positive; a fetch that legitimately picks an un-hardcoded read verb (`search_events`) → still ok; `respond_to_event`/`cancel_or_delete_event`/`create_event`/`send_chat_message` each individually present → HALT; an off-scope connector namespace (email, not calendar/Teams) → HALT; a non-`codex_apps` server (e.g. a stray `github` tool) → HALT; `_scan_partial_output_for_writes()` on a clean partial transcript → no raise, on a transcript containing a write call → raises, on empty/garbage input → no raise (never a false HALT). **Run locally this session: 11/11 pass.**
3. **`lane_b_cal_guard.py` -- `cmd_run()` redesigned (commit `6a681bd`):** no longer takes PRE/POST snapshots or diffs them. Now just runs `lane_b_call1.py --domain calendar` once and trusts its exit code (1=halt/re-contamination, 3=transient/unavailable, 0=clean). Module and function docstrings rewritten to describe the new design plainly. `take_snapshot()`/`diff_snapshots()`/`--snapshot`/`--diff`/`--dry-diff`/`--selftest` are **unchanged and remain available as standalone diagnostic commands** (useful for spot-checking connector-read normalisation) -- they are simply no longer part of what a live scheduled run goes through. `take_snapshot()` also now handles `ReContaminationDetected` specially for consistency, even though it's diagnostic-only now.
4. **`docs/LANE_B_NO_COM_FALLBACK_PROPOSAL.md` updated (commit `a59eb3f`):** the earlier "split exit-1 into 1a/1b, escalate with a confirmation POST2 snapshot" section is now moot -- there's no more ambiguity to resolve by reading again, since every HALT is unambiguously a directly-observed re-contamination event. Simplified: HALT is always a hard stop, no escalation; the "genuinely can't verify" / no-COM-fallback design now applies to HALT (always) and the existing transient/unavailable exit-3 case, unchanged in substance from the original proposal, just simpler with the diff-ambiguity case gone. Still a proposal, nothing implemented from it.

**Verified locally, real python interpreter (not the sandboxed `python3` stub -- see prior entry's environment note):** both files `py_compile` clean; `lane_b_call1.py --selftest` 11/11 pass; `lane_b_cal_guard.py --selftest` 7/7 pass (the pre-existing diff-logic selftest, untouched, still green -- confirms the diagnostic-only snapshot/diff code itself wasn't broken by this change, only unplugged from the live gate); `lane_b_cal_guard.py` imports cleanly and `cmd_run` is callable. **Not live-tested against a real codex/connector this session** -- no RDP access.

## Where everything stands
- **The live safety design is now simpler and, on the evidence, more reliable**: one connector read per cycle instead of two that have to agree, no timing sensitivity, direct tool-call observation instead of inferred state-diffing, and the one real gap in that direct observation (partial/timed-out output) is now closed.
- Write-safety hardening from earlier today (toast, `SAFETY_RULE` prompt hardening, `approval_policy` source-level proof) is unaffected, still on `#31`.
- The quiet-gap fix (`WI_LANE_B_SNAPSHOT_GAP_S`, commit `d6a74de`) is still in the code and still valid for Call-1's own retry behaviour and the now-diagnostic-only `--dry-diff`/`--snapshot` commands -- just no longer load-bearing for the live gate, since there's no more PRE/POST pair in `cmd_run()` to space out.
- Nothing merged. Live task/`CAL_BACKEND` untouched. Calendar stays on COM.

## Next action -- resume here
Re-pull fresh from `#31` (not main, same discipline as every prior gate) and run BOTH selftests plus a real `--run` to confirm the new design end-to-end against the actual connector (the one thing not yet live-tested):
```powershell
cd $env:USERPROFILE\work-inbox
$cb = [DateTimeOffset]::UtcNow.ToUnixTimeSeconds()
iwr -UseBasicParsing "https://raw.githubusercontent.com/begb0037admin/work-inbox/drew/lane-b-cal-guard-snapshot-fix/lane_b_call1.py?t=$cb" -OutFile lane_b_call1.py
iwr -UseBasicParsing "https://raw.githubusercontent.com/begb0037admin/work-inbox/drew/lane-b-cal-guard-snapshot-fix/lane_b_cal_guard.py?t=$cb" -OutFile lane_b_cal_guard.py
...
python .\lane_b_cal_guard.py --run
```
Expect `--run` to be faster and simpler than before (one connector read, not two) and to no longer be exposed to the PRE/POST timing issue at all. If `--run` exits 0: the new design is confirmed live, `#31` is ready for Kevin's own merge/cutover decision (still not automatic -- his explicit go-ahead, same as every prior gate). If it exits 1: check whether it's a genuine write-tool observation (real HALT, investigate immediately, do not dismiss) versus anything else unexpected.

**Standing architectural decision (unchanged, do not re-litigate):** no permanent COM fallback for Lane B calendar. Connector is the mandatory path.
...
## What Kevin asked for this session (2 parts, escalated mid-session)
1. Quick concrete fix: an immediate, hard-to-miss notification the moment the Lane B guard trips (snapshot diff OR `lane_b_call1.py`'s re-contamination guard), reaching Kevin even when he's away from the laptop -- not just a disabled task he discovers later.
2. Investigate (don't build): does Oxford's enterprise-forced `approval_policy` (Never disallowed, forced to UnlessTrusted) actually gate `codex_apps` connector tool calls in headless `codex exec`, or only shell/file actions?

Mid-session, Kevin added two more, both folded into the same branch:
3. Prompt-level hardening: Lane B prompts must never call decline/respond/cancel/create/send tools under any circumstance; if a write is ever unavoidable, zero attendees/recipients. Explicit defense-in-depth, not a technical guarantee.
4. Narrowed threat-model priority: audit today's real `--dry-diff` run (POST snapshot: `list_events` did not fire on attempts 1-2, fired on attempt 3) to confirm no off-allowlist tool was ever attempted on a retry, and confirm the re-contamination guard checks every attempt, not just the final one.

## 1. Cross-machine guard-halt toast -- BUILT, PUSHED to branch #31 (`36b4e8f`, `be50a8d`), NOT merged
Mechanism: reused the EXISTING cross-machine channel already built for briefing/draft-diff failures (`Push-LaptopRunStatus.ps1` -> `data/laptop_status/briefing_status.json` on GitHub -> `Watch-BridgeBriefing.ps1`, the desktop's existing 5-min poll-and-toast scheduled task) -- no new mechanism designed, just a new field threaded through it.

- `Run Laptop Bridge Briefing.ps1`: new `$LaneBGuardResult`/`$LaneBGuardDetail` script vars, set in the guard switch (0=clean, 1=halted, 3=transient, default=unexpected-N), passed to `Publish-Status`. **The actual gap this closes:** a guard HALT (exit 1) falls back to `CAL_BACKEND=com` and the briefing still ships that cycle, so `fetch_inbox.py` exits 0 and the OLD `Publish-Status $rc` call reported "ok" -- the cross-machine channel never even knew a HALT happened. Local BurntToast on the laptop (built 1 Sept) still fires too, wording tidied (removed a duplicated "disabled ... disabled" phrase, follow-up commit `be50a8d`).
- `Push-LaptopRunStatus.ps1`: new `-LaneBGuard`/`-LaneBGuardDetail` params, written into the pushed JSON as `lane_b_guard`/`lane_b_guard_detail` -- still just a short fixed-vocabulary string, no email/calendar content, same privacy bar as the rest of that file.
- `Watch-BridgeBriefing.ps1` (desktop watcher): new toast case, fires on `lane_b_guard=='halted'` INDEPENDENTLY of the existing `result=='failed'` case (so it fires even though the overall run reports ok). Title "Work Inbox - Lane B calendar guard HALTED (laptop)", body = the ts + `$LaneBGuardDetail` (e.g. "task 'Work Inbox Bridge Briefing' disabled; calendar fell back to COM this cycle. See data\lane_b\ and data\codex_runs\GUARD_TRIPPED_* on the laptop.").
- **Not live-fired this session** (no RDP access, and firing it for real would need an actual guard trip -- did not attempt). Description of what it looks like given to Kevin in the chat report instead of a screenshot, per the coordinator's earlier instruction (no genuine blocker, reused an existing proven mechanism).
- **Deployment gap, flagged in the PR #31 comment too:** the wrapper's own self-refresh list (`fetch_inbox.py, imap_mail.py, reauth_imap.py, normalise_pull.py, lane_b_call1.py, lane_b_cal_guard.py`) does NOT include `Push-LaptopRunStatus.ps1` or the wrapper itself -- these need a manual re-pull to the laptop, and `Watch-BridgeBriefing.ps1` a manual re-pull to the desktop (`D:\OneDrive - lelitte.com\Desktop\Watch-BridgeBriefing.ps1`), before this actually fires live. Since the live task never passes `-CalBackend connector`, none of this executes on the regular schedule regardless of redeploy status -- low urgency, but needed before the next manual Lane B test/cutover attempt.

## 2. approval_policy / connector-tool-scope investigation -- ANSWERED at the source level, not by live testing
**Re-verified true:** Oxford's enterprise `requirements.toml` still forces `approval_policy` away from `Never` (`UnlessTrusted`) -- matches the 1 Sept printed message and the current `codex-rs/exec/src/lib.rs` logic (`build_exec_config`): `codex exec` defaults to `AskForApproval::Never` in headless mode, UNLESS the resolved `approvals_reviewer == AutoReview` (Kevin's own `config.toml` has `approvals_reviewer = "auto_review"`), in which case it rebuilds WITHOUT that Never override and the enterprise-forced value applies instead.

**Read the actual public openai/codex Rust source** (`codex-rs/core/src/mcp_tool_call.rs`, `codex-rs/exec/src/lib.rs`, current `main`; cross-checked against 0.149.0's release notes dated 20 Aug, which already reference this MCP-approval/Guardian machinery, so it's plausible this predates Lane B's testing window -- NOT confirmed the exact deployed 0.149.1/0.151.0/0.152.0 binaries share the identical code path, that would need a per-tag source diff not done this session):
- A connector ("Apps") tool call that is flagged as requiring approval (`requires_mcp_tool_approval`: any tool without `read_only_hint=true`, using `destructive_hint`/`open_world_hint` defaulting to TRUE -- i.e. write-capable tools default to "needs approval") DOES raise a real MCP elicitation approval request when `approval_policy != Never`.
- `codex exec` (headless) has NO interactive channel for this. Its own event-loop handler explicitly says (comment in source): "Exec auto-cancels elicitation instead of surfacing it interactively" -> returns `McpServerElicitationAction::Cancel`.
- That Cancel resolves to `ReviewDecision::Abort` in `mcp_tool_call.rs`, which is handled as: "user cancelled MCP tool call" -> the tool call is SKIPPED, never executed -- the model gets an error string back, not a real write.

**So: yes, structurally, on the current public codex-cli source, a genuine write-capable connector tool call attempted inside a headless Lane B run on the Oxford Edu account (approval_policy forced off Never) would be auto-aborted rather than executed -- this is real prevention, not just after-the-fact detection, and meaningfully upgrades the 28-29 Aug NOT SOUND verdict (`docs/CONNECTOR_SAFEGUARDS.md`), which predates this finding.**

**But two real caveats, not resolved this session:**
1. This is gated on the tool's `AppToolApproval` mode, which for `codex_apps` comes from `AppToolPolicyEvaluator` -- effectively the CHATGPT ACCOUNT'S OWN per-connector "Action control" setting (Settings -> Connectors -> [connector] -> Action control: always ask / read-only / custom / always allow), NOT just the session `approval_policy`. If that setting is permissive (`AppToolApproval::Approve`), `requires_mcp_tool_approval_for_mode` returns false UNCONDITIONALLY and the whole Abort path above never engages -- `approval_policy` is irrelevant. **This plausibly explains the 26-27 Aug incident** (a real `set_message_categories` write fired headless with zero prompt, on the personal ChatGPT Plus account) without contradicting the source-level finding above: that account's connector Action-control setting was very likely permissive, and/or that account isn't enterprise-managed so `approval_policy` may default to `Never` there regardless. **Not checked this session:** what Oxford Edu (`begb0037@ox.ac.uk`) or the Lane B dedicated personal-account login (`C:\WorkInboxAI\codex-laneb`) currently have set for Outlook Calendar/Teams Action control. This is a SAFE, zero-risk thing to check directly in ChatGPT Settings -> Connectors -- no codex run needed. Recommend Kevin (or a future session) check this before leaning on the Abort protection.
2. **Cannot verify live without risking a real write** (explicitly told not to attempt this). The whole chain above is source-code-derived, never empirically observed for an actual write attempt in Lane B (every real Lane B prompt has only ever asked for reads). Existing guard/kill-switch layers (re-contamination verb-check, calendar snapshot diff, now the SAFETY_RULE prompt hardening) remain necessary defense-in-depth regardless of whether this Abort path is confirmed live, precisely because it hasn't been.

## 3. Kevin's connector tool-schema question ("silent/no-notify param on decline/cancel/respond?") -- UNRESOLVED, could not check from here
Checked this desktop's own `~/.codex/.codex-global-state.json` connector catalog (158 `codex_apps` tools, refreshed 1 Sept) -- it currently has ZERO write tools for `microsoft_outlook_calendar`/`microsoft_outlook_email`/`microsoft_teams` at all (no `create_event`, `cancel_or_delete_event`, `respond_to_event`, etc. -- only list/get/fetch/search/resolve). This is a DIFFERENT catalog state than the 29 Aug 113-tool/49-write enumeration, and it's this DESKTOP's account, not Lane B's dedicated `C:\WorkInboxAI\codex-laneb` login on the laptop -- couldn't inspect that one's catalog from here. Could not confirm or rule out a "silent"/no-notify parameter on `respond_to_event`/`cancel_or_delete_event` from a live manifest. (General MS Graph API knowledge: the underlying `acceptEvent`/`declineEvent`/`tentativelyAcceptEvent` operations do support a `sendResponse: false` parameter -- NOT confirmed whether the ChatGPT connector's tool schema exposes this to the model.) The core rule stays "never decline/cancel/respond at all" regardless -- this was only ever meant as belt-and-braces if unavoidable. Folded the "if ever unavoidable, zero attendees" framing into SAFETY_RULE instead of a specific silent-param instruction, since the param's existence is unconfirmed.

## 4. Today's --dry-diff retry-attempt audit -- NO raw transcripts exist to grep; code-level proof given instead
`data/codex_runs/` and `data/lane_b/` do not exist on `main` at all (confirmed via the Contents API, 404) -- Lane B's `--snapshot`/`--dry-diff`/`--run` codepaths never persist the raw `codex exec --json` transcript to disk (unlike `lane_b_call1.py`'s `fetch_domain()`, which DOES write `{ts}_call1_{domain}_a{n}.jsonl` per attempt -- `lane_b_cal_guard.py`'s `take_snapshot()` has no equivalent). So a literal grep of today's actual attempts is not possible from here or anywhere -- those logs only ever existed in the laptop's PowerShell console during Kevin's RDP session and were never captured to a file.

**Structural (code-level) proof instead, confirmed by reading `take_snapshot()` in `lane_b_cal_guard.py`:** `guard_recontamination()` is called INSIDE the retry loop on every single attempt (not just the last), immediately after each `run_codex_json()` call, BEFORE deciding ok/unavailable/halt for that attempt. Any off-allowlist or write-verb tool call on ANY attempt -- including a discarded/failed retry -- raises `RuntimeError` immediately, which `cmd_run()`/`cmd_dry_diff()` surface as a distinct "re-contamination guard tripped" HALT (exit 1, different log line) -- NOT the "NOT CLEAN residual diff" outcome that was actually observed and recorded in the ~00:1x entry below (8 added all-day events). Since the observed outcome was the diff path, not the re-contamination path, this is solid indirect proof that `guard_recontamination` returned ok or unavailable (never halt) on all 3 PRE/POST attempts today -- i.e. no unexpected/write tool was ever called on any attempt, including the two that failed to return `list_events`. Same structure confirmed in `lane_b_call1.py`'s `fetch_domain()`/`run_domain()` -- every attempt is checked, not just the final one. **No code change needed for this specific concern** -- the guard already covers every attempt. The one real gap: these per-attempt results aren't PERSISTED anywhere (only a terse "list_events did not fire" log line for an unavailable attempt, no record of which tools WERE seen) -- worth a small follow-up (write `take_snapshot()`'s per-attempt `tool_calls` list to `data/codex_runs/` the way `fetch_domain()` already does) if Kevin wants future runs auditable, not done this session (out of the timeboxed scope Kevin set).

## Next action
1. Kevin/future session: check ChatGPT Settings -> Connectors -> Outlook Calendar / Teams -> Action control for both the Oxford Edu account and the Lane B `C:\WorkInboxAI\codex-laneb` personal login -- confirms or rules out whether the source-level Abort-on-write protection ("2" above) is actually armed. Zero risk, no codex run needed.
2. Still outstanding from the ~00:1x entry below: isolate the --dry-diff "8 added all-day events" gap (connector-completeness vs. `_natural_key`/`_fingerprint_one` bug) before any cutover retry -- untouched this session.
3. Redeploy `Push-LaptopRunStatus.ps1` + the wrapper to the laptop, `Watch-BridgeBriefing.ps1` to the desktop, before the next manual Lane B test run, so the new toast wiring actually has effect.
4. Optional follow-up: persist `take_snapshot()`'s per-attempt tool-call list to `data/codex_runs/` for future auditability (not urgent, no live risk either way).
...
Kevin's directive for the 1 Sept evening was **"everything tonight"** -- an explicit one-time override of the standing work-inbox cautious-change-pace rule (small isolated verified changes on the live pipeline; see the 17 Aug memory entry). That override applied to the evening only. **Change pace is now back to normal** -- the three PRs below stay DRAFT and gated on Kevin's review, feed-fix merges before the CC PR, and any dashboard change still needs a screenshot + Kevin's literal "approved".

## 1. LANE B CALENDAR CUTOVER -- ATTEMPTED, FALSE-TRIPPED, ROLLED BACK

`docs/desktop-scripts/Cutover Lane B Calendar.ps1` was run in **Kevin's RDP session** (`AD-OAK\begb0037` on `101L-DE013193`).

**Preconditions PASSED:**
- Lane B codex `CODEX_HOME=C:\WorkInboxAI\codex-laneb` signed into **`kevin@lelitte.co.uk`**, plan **`plus`**, account **`eb7a812e-1b9d-4586-b1a4-02a4ed7ca116`** (personal, not Edu).
- Connector calendar probe returned **41 real events**.
- Backups written on the laptop:
  - `Run Laptop Bridge Briefing.ps1.bak-20260901-204854` (the live wrapper, path parsed from the task action).
  - `logs/WorkInboxBridgeBriefing_task_20260901-204854.xml` (`Export-ScheduledTask`).

**Then `lane_b_cal_guard.py` exited 1 -- persistent HALT:**
> `quarantined lane_b_normalised.json ... (52 calendar change(s) during read window)`

**This was a FALSE POSITIVE.** Two back-to-back `list_events` reads of an unchanged calendar diffed by ~52 because:
1. Recurring-series **occurrences** get a fresh Graph `id` on every connector call -> each occurrence read as `removed` + `added`.
2. `start` / `end` were compared as the raw `.isoformat()` of whatever timezone label the connector rendered that call.

**The guard failed CLOSED and correctly:** it ran `Disable-ScheduledTask` on `Work Inbox Bridge Briefing`, fell back to `CAL_BACKEND=com` for that cycle, and **the briefing still shipped on COM (commit `bede48d`)**. `BurntToast` is not installed on the laptop, so the toast step was skipped (try/catch, non-fatal, as designed).

## 2. RECOVERY -- same night, live task is now unambiguously COM

Done in the same RDP session:
- `Register-ScheduledTask -Xml (Get-Content '<...WorkInboxBridgeBriefing_task_20260901-204854.xml>' -Raw) -TaskName 'Work Inbox Bridge Briefing' -Force`.
- Live wrapper restored from `Run Laptop Bridge Briefing.ps1.bak-20260901-204854`.
- **`-CalBackend connector` stripped from the task action.** The rebuilt action was constructed **without `-WorkingDirectory`** because the task's own `$a.WorkingDirectory` is empty -- passing an empty `-WorkingDirectory` to `New-ScheduledTaskAction` would have thrown.
- Verified read-back: task **State: Ready**, **NextRunTime 2026-09-02 07:00**, action arguments contain **no** `-CalBackend connector` (COM is the default) -> unambiguously on Outlook COM.

**Lane B calendar connector = PARKED** pending the guard fix (§4, PR #31). Do **not** re-attempt the connector cutover until a fresh RDP `--dry-diff` is clean.

## 3. COMMAND CENTRE FEED BUG -- ROOT CAUSE CONFIRMED + LIVE-REPRO'D

**Symptom:** no new Command Centre cards since ~27 Aug; task-update bullets collapsed to ~1 per task, lifetime.

**Root cause (confirmed):**
- Under `MAIL_BACKEND=imap` **every email has `entry_id=""`** -- by design (`imap_mail.py` ~L486 / ~L773; there is no Outlook COM EntryID under IMAP).
- `fetch_inbox.py` Phase 3.6 promotion guard **`if not eid: continue` (~L3062)** then silently skips **100%** of new-task candidates.
- The `applied` ledger key `entry_id + "_" + task_id` degenerates to **`_<task_id>`**, so each task gets exactly **one** inbox update ever, then locks.

**Live proof (1 Sept eve):**
- `triage_ledger.json` `promoted` map frozen at **2026-08-27** (49 real-EntryID keys); only **13** `_`-prefixed `applied` keys since the IMAP cutover on 30 Aug.
- Tonight's bridge run logged **`Phase 3.5 done - new:3 ... updates:0`**, **no Phase 3.6 line**, **no `tasks.json` commit**; the 3 new tasks dropped to unactioned suggestions (Phase 5, commit `29ea61e`).
- This was a **known-deferred follow-up** from the 28 Aug IMAP build ("key on `message_id`") that never got done before the 30 Aug cutover.

## 4. FIXES -- ALL ON PUSHED BRANCHES, DRAFT PRs OPEN, NOTHING MERGED

### 4a. work-inbox feed-fix -- branch `drew/cc-feed-imap-identity-fix` @ `c5f52db7` -- [PR #32](https://github.com/begb0037admin/work-inbox/pull/32) (DRAFT)

| File | Change | Evidence |
|---|---|---|
| `fetch_inbox.py` | **+65 / -12.** New module-scope helpers `_cc_mail_key(entry_id, message_id)` (`entry_id or message_id or ""`) and `_owa_link()`. Phase 3.5 dedup + Phase 3.6 promote-guard both use `_cc_mail_key`; `applied` **and** `promoted` ledger keys migrated read+write; promoted cards also carry `messageId` + `webLink`; `entryId` no longer clobbered with `""` in the apply loop; new `messageId` dedup set guards re-promotion. | `py_compile` OK; COM-path keys **byte-identical** to pre-fix for real EntryIDs. |
| `test_cc_mail_key.py` | **NEW, +99.** AST-extracts the real `_cc_mail_key` / `_owa_link` from `fetch_inbox.py` (no import side effects), asserts COM key parity + IMAP `Message-ID` fallback + empty-identity skip guard + OWA link host. | **10/10 pass** locally. |
| `clean_triage_ledger.py` | **NEW, +148. BUILT, NOT RUN.** One-time removal of the 13 poisoned `_`-prefixed `applied` keys. GitHub-API only, **dry-run by default** (dry-run: 385 -> 372); `--apply` does backup -> byte-verify -> write -> post-write verify. `promoted` / `tracked_needs_urgent` never touched. | dry-run output only so far. |

### 4b. Lane B calendar guard fix -- branch `drew/lane-b-cal-guard-snapshot-fix` @ `505c30790068a0b3c060c2b7de3dca34dd30f35e` -- [PR #31](https://github.com/begb0037admin/work-inbox/pull/31) (DRAFT)

**Split out of `drew/cc-feed-imap-identity-fix` on Drew's recommendation** so the guard fix has its own review track. Branched from `main` HEAD `2ce7575`; single commit; `lane_b_cal_guard.py` blob `5599f3fe384cf39b23f4084d573938ab6e16cafb` is **byte-identical** to the copy on the feed-fix branch.

`lane_b_cal_guard.py` **+233 / -29:**
- Match events on a **stable natural key**: `iCalUID` (else `id`, else `subject`) **+ the start instant canonicalised to UTC** (the instant disambiguates the occurrences of one series).
- Compare only load-bearing fields -- `subject` / `start` / `end` / `response_status` / `all_day` -- each normalised (datetimes -> UTC ISO-8601 `Z`; all-day -> bare date; `response_status` casefold + alias; `subject` whitespace-collapsed). `type` no longer compared.
- Pin **one** 7-day window per guard cycle, passed to PRE and POST (was recomputed each call -> a cycle straddling UTC midnight shifted the window).
- `lane_b_call1.py` re-contamination guard (server must be `codex_apps`; write/unknown verb -> HALT) is **completely untouched**. A real `subject` / `start` / `end` change or a genuine add/remove still HALTs.
- New: `--dry-diff` (PRE+POST back-to-back, no Call-1, assert 0 diff -- the cheap pre-cutover validator) and `--selftest` (pure-function, no codex).

Evidence: `--selftest` **7/7** locally (incl. the id-churn + tz-label + case-churn case -> 0 trips, and a genuine change -> trip); `py_compile` OK. **NOT integration-tested against a live connector** -- needs an RDP `--dry-diff` run as `AD-OAK\begb0037` on `101L-DE013193` before any connector cutover retry.

Residual (documented inline in the file): an intermittent Windows-tz-*label* flip with no offset in the `dateTime` string is only fully normalised where `zoneinfo` / `tzdata` resolves the zone; otherwise wall-clock is treated as UTC (symmetric across both snapshots). `--dry-diff` surfaces any residual before a cutover retry.

**PR #32 still physically carries this same file** (the split was done by copying, not by reverting it from the feed-fix branch). A PR comment on #32 records that it will be reverted from that branch **on Kevin's word** -- not force-pushed pre-approval, to avoid a force-push race.

### 4c. command-centre board-UX + IMAP opener -- branch `drew/cc-imap-open-email-and-board-ux` @ `656694ba` -- [PR #15](https://github.com/begb0037admin/command-centre/pull/15) (DRAFT)

- `js/app.js` **+53 / -4:**
  - `renderBoard` sorts each tier by most-recent-activity desc (`dateAdded` / genuine action stamps via `lastActivityTs`); new + updated cards land at the top. **This supersedes manual intra-tier drag order.** Between-tier drag + Move buttons are **unaffected**.
  - `cardsWithDayDivider`: one **"Earlier"** divider between cards active today (local midnight = Work Inbox's briefing day boundary) and older cards. **Only renders when a tier has both today-active and older cards.**
  - `cardHTML`: new branch -- a task with `webLink` / `messageId` but no real `entryId` (IMAP-sourced inbox tasks) gets the `openEmailWeb()` button. `openEmailWeb()` also reads camelCase `webLink` and synthesises an OWA search link from `messageId`; same `https` + `outlook.office.com` / `outlook.office365.com` host allowlist.
- `css/styles.css` **+6:** `.board-day-divider` rule.
- Two archived pre-edit backups on the branch (`Archive/app.js.20260901T200221Z.bak`, `Archive/styles.css.20260901T200221Z.bak`), byte-verified identical to the live files before editing.
- `node --check js/app.js` OK.
- Inert until the feed-fix (§4a) lands -- no IMAP card carries `webLink` / `messageId` until then.

### Screenshots for Kevin (in `C:\Users\admin\Downloads\`)
- `CC-dashboard_20260901_1_live-data_newest-first.png` -- CC, live data, newest-first sort.
- `CC-dashboard_20260901_2_demo_earlier-divider.png` -- CC, demo data, the "Earlier" divider.
- `wi-datesort-move-1sep\01-BEFORE.png` / `02-AFTER.png` / `03-AFTER-move-menu-open.png` -- Work Inbox dashboard Move-button bundle (see NEXT (f)).

## 5. EXACT NEXT ACTIONS

(a) **DONE this checkpoint** -- the 3 draft PRs are open: feed-fix [work-inbox #32](https://github.com/begb0037admin/work-inbox/pull/32), Lane B guard [work-inbox #31](https://github.com/begb0037admin/work-inbox/pull/31), CC board-UX [command-centre #15](https://github.com/begb0037admin/command-centre/pull/15). PR comment on #32 records the guard-file overlap with #31.

(b) **Kevin reviews the CC screenshots** (`CC-dashboard_20260901_1_*` and `_2_*` in `C:\Users\admin\Downloads\`) and replies the literal word **"approved"** -- standing UI approval gate.

(c) **Merge order: feed-fix PR #32 FIRST, THEN CC PR #15.** The CC board changes consume `messageId` / `webLink` fields that #32 is what starts writing.

(d) **After #32 merges:** run `clean_triage_ledger.py` -- dry-run first, then `--apply` -- in the RDP session as `AD-OAK\begb0037` on `101L-DE013193`.

(e) **Lane B guard (PR #31):** run `python lane_b_cal_guard.py --selftest`, then a fresh `python lane_b_cal_guard.py --dry-diff` in the RDP session. **Only if `--dry-diff` is CLEAN**, retry the connector cutover (`docs/desktop-scripts/Cutover Lane B Calendar.ps1`). The live task stays on `CAL_BACKEND=com` until then.

(f) **Work Inbox dashboard Move-button bundle:** there is **NO branch and NO commit** for this -- a prior agent's code was lost. Rebuild it from the screenshots in `C:\Users\admin\Downloads\wi-datesort-move-1sep\` (`01-BEFORE` / `02-AFTER` / `03-AFTER-move-menu-open`). Needs a fresh screenshot + Kevin's literal "approved" before any push.

(g) **Teams briefing section in `fetch_inbox.py`:** only after the calendar connector is actually live (i.e. after (e) succeeds). `lane_b_call1.py --domain teams` already emits the normalised Teams block; still needs the briefing-section shape + LANE_B sec.6b's softer "disable NEXT run" Teams guard.

## 6. LIVE STATE AT THIS CHECKPOINT (verified)

- `Work Inbox Bridge Briefing` (laptop `101L-DE013193`, `AD-OAK\begb0037`): **State Ready**, `MAIL_BACKEND=imap`, `CAL_BACKEND=com` (no `-CalBackend connector` in the action), NextRunTime **2026-09-02 07:00**. **Not touched by this checkpoint.**
- work-inbox `main` HEAD `2ce7575de554e344e424bd0f82e7143b7d0da436`; command-centre `main` HEAD `1cfb9a77c135b4b9cf7dcb21486ae7008c96fa90`. **No merges.**
- Branches: `drew/cc-feed-imap-identity-fix` @ `c5f52db7`, `drew/lane-b-cal-guard-snapshot-fix` @ `505c3079`, `drew/cc-imap-open-email-and-board-ux` @ `656694ba`.
- Last good `data/briefing.json` = the COM-fallback run that shipped during the aborted cutover, commit `bede48d`.

---

# Handover -- 1 September 2026, ~16:35 UTC (Drew) -- LANE B (connector calendar + Teams): SINGLE CHECKPOINT. [SUPERSEDED by the 1 Sept ~20:30 UTC entry above -- the connector calendar cutover was ATTEMPTED that evening and ROLLED BACK on a guard false-positive; the live task is back on CAL_BACKEND=com.] **REAL PUSHED BRIEFING WITH REAL CONNECTOR CALENDAR DATA** -- 1 Sept ~16:08-16:14, `101L-DE013193` as `AD-OAK\begb0037`: `lane_b_call1.py --domain calendar` (via `-Fast`) returned 53 real events, then `fetch_inbox.py MAIL_BACKEND=imap CAL_BACKEND=connector AI_BACKEND=claude_code` logged "Phase 1 - Lane B connector calendar: 53 event(s)" and pushed briefing commit [`0d78be5`](https://github.com/begb0037admin/work-inbox/commit/0d78be5) + task-suggestions commit [`ba94e58`](https://github.com/begb0037admin/work-inbox/commit/ba94e58), exit 0 throughout. **This run was on the Edu account** (before Kevin's personal-account switch, §0 below) -- do not confuse it with the personal-account go-live, which is still pending Kevin's 2 prerequisites. Full transcript: `C:\Users\begb0037.AD-OAK\work-inbox\logs\lane_b_test_20260901-160854.log` (on the laptop, not in the repo). **The bridge wrapper is guard-aware** (§3a) and **`docs/desktop-scripts/Cutover Lane B Calendar.ps1` (§3c) is a one-shot cutover script** -- preconditions + backup + flip + re-register + force-run-verify + printed rollback. **PERSONAL-ACCOUNT PROBE CONFIRMED GREEN (1 Sept ~17:09):** `CODEX_HOME=C:\WorkInboxAI\codex-laneb` -> id_token email `kevin@lelitte.co.uk`, plan `plus`, account `eb7a812e-...`; the Outlook Calendar connector IS attached to personal; `-Fast -Retries 1` pulled **52 real events** (status ok, 1 attempt, ~2.5 min) and pushed briefing [`8ee7adf`](https://github.com/begb0037admin/work-inbox/commit/8ee7adf) (mail 46, cal 52, exit 0), guard did not false-trip. Kevin has the **cutover go-ahead** -- runs `Cutover Lane B Calendar.ps1` tonight to be live for 7am. Until then the live task stays on `CAL_BACKEND=com`. **IDENTITY PIVOT (Kevin, ~15:30): Lane B moves off the Oxford Edu account to Kevin's PERSONAL ChatGPT account, via a dedicated `CODEX_HOME`** (Edu's 500/month hard credit cap can't sustain Lane B + Kevin's interactive use -- 272/500 burned on day one). Remaining blockers: (a) Kevin's 2 prerequisites in §0; (b) a repeatable execution path into the `AD-OAK\begb0037` context on `101L-DE013193`; (c) Teams briefing section, not yet built. All older Lane B entries (three "MAKE-OR-BREAK failures", "abandon Edu", "Edu primary / personal failover", "attended route", pre-16:35 versions of this entry) are SUPERSEDED and kept only for audit.
...
- **ROOT CAUSE of the `-RealBriefing` hang (every `codex exec --json` attempt timed out 3x): `lane_b_call1.py` spawned codex without closing stdin.** codex-cli 0.151.0 prints "Reading additional input from stdin..." on every `codex exec` and BLOCKS reading stdin until EOF; under `powershell -File ... -> python -> subprocess.run` the inherited stdin never closes -> hang. **FIXED**: `stdin=subprocess.DEVNULL` added to the `subprocess.run` in `run_codex_json()` AND `_ensure_warm()`. Prompt stays the trailing argv element -- NOT piped via stdin. A manual `& codex exec ... "<prompt>"` in a real console always worked (real stdin -> EOF) -- that was the tell. Real calendar call ~3-3.5 min; per-call timeout now 360s (`WI_LANE_B_TIMEOUT` / `WI_LANE_B_SNAP_TIMEOUT`). The warm-up is kept but is near-useless now (the hang was never cold-start).
- **`~/.codex/config.toml` sha1 check is now "is it a recognised good hash"** (any recorded host, or `WI_CODEX_CONFIG_SHA1`) -- host-key case can't cause a false alarm. Recorded: `101l-de013193` = `ba0184e864ffd081069820cc7a6f8f19acf5c845` (AD-OAK\begb0037, Edu, 0.151.0), `desktop-mjdjm64` = `4fd8ef763bf0a8ddad9a138b6679a84fe8536f73`. A truly-unknown hash logs a "record it" note, **never a HALT**. The run also logs `host=<x> ~/.codex/config.toml sha1=<y>` so the next run shows what it resolved.
...
| `docs/desktop-scripts/Run Lane B Calendar Test.ps1` | **THE FALLBACK EXECUTION PATH** (see sec.4). Self-contained PowerShell 5.1, pulls the scripts fresh from `raw` main, prints a "PASTE THIS BACK" summary. **`-Fast`** = the ~6 min one-time proof: step 1 -> `lane_b_call1.py --domain calendar` -> (if ok) `fetch_inbox.py` connector briefing; **SKIPS the snapshot guard** (that layer is validated when it's wired into the bridge wrapper; `lane_b_call1.py`'s own verb-based re-contamination guard still runs). Full run (no `-Fast`) = steps 1-4 incl. `lane_b_cal_guard.py --run`, ~12-15 min (codex ~3 min/call, the guard adds 3 more calls). `-Retries 1` to fail fast on a flaky connector. Kevin runs it in his RDP session as `AD-OAK\begb0037`. |

Tested: green offline against the coordinator's real 31.5 KB probe; guard verb-classification unit-tested; end-to-end on the real machine 1 Sept ~14:57 (mechanism proof); **and 1 Sept ~16:08-16:14, a REAL PUSHED BRIEFING with 53 real connector calendar events** -- `lane_b_call1.py --domain calendar` status ok (one attempt, ~4.5 min) -> `fetch_inbox.py MAIL_BACKEND=imap CAL_BACKEND=connector AI_BACKEND=claude_code` -> "Phase 1 - Lane B connector calendar: 53 event(s)" -> briefing pushed [`0d78be5`](https://github.com/begb0037admin/work-inbox/commit/0d78be5), suggestions pushed [`ba94e58`](https://github.com/begb0037admin/work-inbox/commit/ba94e58), exit 0 throughout. On Edu (pre-pivot).
`data/lane_b/` + `data/codex_runs/` are NOT committed (`data/` is gitignored wholesale; the runner `mkdir -p`s them).

## 3a. Bridge-wrapper glue -- BUILT, DORMANT (live task still `CAL_BACKEND=com`)

`docs/desktop-scripts/Run Laptop Bridge Briefing.ps1` (the script the live `Work Inbox Bridge Briefing` scheduled task actually runs) is now guard-aware:
- Refresh loop pulls `normalise_pull.py` + `lane_b_call1.py` + `lane_b_cal_guard.py` fresh from `main` alongside the existing 3 mail scripts.
- **Only when `-CalBackend connector` is explicitly passed** (the registered task does not pass it -> dormant on the live schedule): runs `lane_b_cal_guard.py --run` before `fetch_inbox.py`.
  - **exit 0** -> proceed with `CAL_BACKEND=connector` for real.
  - **exit 1 (persistent HALT)** -> `Disable-ScheduledTask -TaskName 'Work Inbox Bridge Briefing'` + a best-effort `BurntToast` (wrapped in try/catch, never fails the run if the module is missing) + falls back to `CAL_BACKEND=com` for **that cycle only** (degrades to empty+warning on this laptop, same as always -- the briefing still ships). No auto re-enable.
  - **exit 3 (transient)** -> logs, falls back to `CAL_BACKEND=com` for that cycle only, **task stays enabled**, retries next cadence.
- **Single identity flip point:** `$LaneBCodexHome` near the top of the script (currently `''` = use whatever codex is already logged into on this session -- Edu, as of this wiring). Flip it to `'C:\WorkInboxAI\codex-laneb'` (one line) once Kevin has done the personal-account `codex login` from §0. `$TaskName` is also a named variable (`'Work Inbox Bridge Briefing'`) for the `Disable-ScheduledTask` call.
- **THE LIVE TASK STAYS ON `CAL_BACKEND=com`.** This is glue only -- cutover (re-registering the task with `-CalBackend connector`) needs Kevin's explicit go-ahead, same discipline as the mail IMAP migration. Proving it end to end is done manually via `-CalBackend connector` on this same script, or via `Run Lane B Calendar Test.ps1 -Fast`.
- PS 5.1 parse-checked clean.

## 3c. Cutover script -- `docs/desktop-scripts/Cutover Lane B Calendar.ps1` (BUILT; run is Kevin's call)

ONE PowerShell 5.1 script Kevin pastes once in his RDP session (`AD-OAK\begb0037` on `101L-DE013193`). Kevin has given the **explicit cutover go-ahead, conditional on the personal-account probe being green** -- the script's own step-1 preconditions enforce that condition, so if the pending `-Fast -Retries 1` validation is slow, this script covers the same ground. Wall time ~25-30 min (connector probe ~4 min + a full guarded briefing run ~15-20 min). Steps:
1. **Preconditions (fail loud, change nothing):** `C:\WorkInboxAI\codex-laneb\auth.json` exists; its `id_token` decodes to email `kevin@lelitte.co.uk` and **not** `begb0037@ox.ac.uk` / plan `education` (prints what it found); a real `lane_b_call1.py --domain calendar` probe under that `CODEX_HOME` returns `status=ok` with `count>0` -- if `unavailable`, STOP + tell Kevin to add the Outlook Calendar connector to his personal ChatGPT account; if `halt`, STOP (a write tool appeared).
2. **Backup:** copies the live `Run Laptop Bridge Briefing.ps1` (path parsed from the task's own action) to `<path>.bak-<ts>`, and `Export-ScheduledTask` -> `logs\WorkInboxBridgeBriefing_task_<ts>.xml`. Prints both.
3. **Flip:** pulls `Run Laptop Bridge Briefing.ps1` fresh from `main`, sets `$LaneBCodexHome = 'C:\WorkInboxAI\codex-laneb'`, writes it to the live path (aborts before touching the live file if the flip doesn't apply cleanly).
4. **Re-register:** `Set-ScheduledTask -Action` (preserves triggers / principal / run-whether-logged-on -- only the action `-Argument` changes: `-CalBackend com` -> `-CalBackend connector`, or appends it). Reads the task back, prints the new action line + the (unchanged) triggers. Idempotent: if `-CalBackend connector` is already there it leaves the args alone.
5. **Force run + verify:** `Start-ScheduledTask`, polls `State` up to `-RunTimeoutMin` (default 22). Prints the guard exit line, the `Phase 1 - Lane B connector calendar: N event(s)` line, the `fetch_inbox.py exit` line, and `data/briefing.json` HEAD sha before/after (GitHub API).
6. **Rollback (printed, not run):** `Register-ScheduledTask -Xml (Get-Content '<xml bak>' -Raw) -TaskName 'Work Inbox Bridge Briefing' -Force`  OR strip the flag via `Set-ScheduledTask`;  plus `Copy-Item '<script bak>' '<live ps1>' -Force`.
7. Ends with a PASTE-THIS-BACK block (precondition results, backup paths, new action line, guard exit, calendar count, briefing commit, run exit, rollback commands, transcript path).

`-SkipRun` does everything but the force-run. Does NOT touch the mail/IMAP path. PS 5.1 parse-checked clean.
Identity checks (belt + braces): id_token must decode to email `kevin@lelitte.co.uk` AND plan in {plus,pro,team} (WARN-only if the plan claim is unreadable); hard STOP on `begb0037@ox.ac.uk` / plan `education`|`enterprise` / account `cc80356f-...`. Also prints `account_id` (personal = `eb7a812e-...`).
**account_id extractor fixed this pass:** `_codex_identity()` in `lane_b_call1.py` (and the PS scripts) now read `tokens.account_id` + decode the id_token's `https://api.openai.com/auth` claim -- previously `_codex_account_id()` logged "(auth.json has no account_id)" because it only checked a non-existent top-level `.account_id`. Every Lane B run now logs `CODEX_HOME=<x> account_id=<y>  email=<z> plan=<p>` so it's always visible at a glance that Lane B is on personal, not Edu.
Raw URL: `https://raw.githubusercontent.com/begb0037admin/work-inbox/main/docs/desktop-scripts/Cutover%20Lane%20B%20Calendar.ps1`

## 3b. Sanity check -- "15 calendar summaries look generic or weak" (not blocking, benign, pre-existing)

Quick check per the coordinator's ask. `weak_calendar_summary()` / `weak_calendar_summary_count()` / the `validate_briefing_update()` warning line (fetch_inbox.py ~880-963) are **pre-existing Phase 3.8 code, unrelated to Lane B** -- they count AI-generated per-event `summary` fields (from the Anthropic/claude Phase 3.8 call) that are short (<45 chars) or match a known-generic phrase list, one of which is literally `"blocked time or placeholder"`. The 53-event real pull included `Busy` / `Keep free` blocks (per the coordinator's own evidence) -- exactly the kind of item this phrase list exists to tolerate; a short/generic AI summary for a "Busy" block is expected, not a defect. It is a **warning only** (never fatal) in `validate_briefing_update()`. Nothing about Lane B changes what Phase 3.8 sees per event (same `subject`/`body_preview` shape as COM). Not Lane-B-introduced; not investigated further per "don't rabbit-hole on it if benign".
...
## Next action
1. **Mechanism is PROVEN, INCLUDING a real pushed briefing with 53 real connector calendar events** (1 Sept ~16:08-16:14, commits `0d78be5`/`ba94e58`, on Edu). No more mechanism doubt.
2. **Bridge-wrapper glue is BUILT (§3a)** -- guard-aware, dormant. **Live task stays on `CAL_BACKEND=com`** until Kevin's explicit cutover go-ahead.
3. **Kevin's 2 prerequisites (§0):** (a) add the **Microsoft Outlook Calendar** connector to his **personal** ChatGPT account (Teams already works there); (b) dedicated **`codex login` into `C:\WorkInboxAI\codex-laneb`** (`$env:CODEX_HOME=` that dir first) signed into **personal** (`kevin@lelitte.co.uk`), not Edu.
4. **Then one validation run:** `docs/desktop-scripts/Run Lane B Calendar Test.ps1 -Fast -Retries 1` in the RDP session on `101L-DE013193` as `AD-OAK\begb0037`. ~6 min; proves connector calendar reaches a real pushed briefing on the **personal** account (the 53-event proof above was still on Edu). Paste back the summary block. Record the Lane B `CODEX_HOME/config.toml` sha1 into `lane_b_call1.py CONFIG_TOML_SHA1_BASELINES`; flip `$LaneBCodexHome` in `Run Laptop Bridge Briefing.ps1` to `C:\WorkInboxAI\codex-laneb` (one line).
5. **CUTOVER (Kevin's call, go-ahead GIVEN conditional on the probe being green):** paste `docs/desktop-scripts/Cutover Lane B Calendar.ps1` once in the RDP session (§3c). It preconditions -> backs up -> flips `$LaneBCodexHome` -> re-registers the task with `-CalBackend connector` -> force-runs + verifies -> prints rollback. After it: the live 07:00/12:00/16:00 Mon-Fri task ships briefings with connector calendar. Rollback is one line, printed by the script.
6. **Then (Drew):** build the **Teams briefing section** in `fetch_inbox.py` (`lane_b_call1.py --domain teams` already emits the normalised Teams block; needs a new briefing-section shape + LANE_B sec.6b's softer "disable NEXT run" Teams guard). Any dashboard/UI change goes through the standing screenshot + Kevin's literal "approved" gate before push -- not skipped for Lane B.
...
- **`data/laptop_status/draftdiff_status.json` @ `main`** = `result: ok`, `exit_code: 0`, `host: 101L-DE013193`, `user: AD-OAK\begb0037`, `mail_backend: imap`, `ai_backend: claude_code`, `drafts_tracked_now: 104`, `draft_final_pairs_found: 0`, `pairs_classified_this_run: 0`, `backlog_size_after_this_run: 0`, `window_hours: 72`, `ts: 2026-09-01T07:30:34+01:00`. This overwrote the `1e2a67c` `pending` placeholder.
- **Commits since the 30 Aug evening handover (`62b9da4`):** `6069fcc` (31 Aug 06:05, `chore: update triage ledger`); `8ef6e35` (31 Aug 07:30), `f19cf35` (31 Aug 12:30), `47d545d` (31 Aug 16:30), `a5b8099` (1 Sep 07:30) -- all `chore: laptop draftdiff run-status (ok, exit 0)`, each a `+2/-2` timestamp bump on the status file. Times are Bridge cadence (07:30/12:30/16:30). No other non-routine commits.
- **All 5 runs show `draft_final_pairs_found: 0` / `pairs_classified_this_run: 0`.** The `claude -p` (`AI_BACKEND=claude_code`) enrichment path on the IMAP backend has STILL never processed a real draft->sent pair -- nothing has left Drafts since the ledger re-baselined. End-to-end correlation->classify remains unproven; only key extraction (`TIDX:` dominant, run #1) + capture + status-push are proven.
- **Scripts on `main` unchanged since 30 Aug evening.** `docs/desktop-scripts/Run Laptop Draft Diff.ps1` + `Push-LaptopRunStatus.ps1` last at `528cc96`; `Register-LaptopDraftDiff.ps1` last at `62b9da4`. Confirmed `Run Laptop Draft Diff.ps1` carries the hashtable-splat fix (`$a = @{ Kind = 'draftdiff'; ExitCode = $code }`) and `AI_BACKEND='claude_code'` default; `Register-LaptopDraftDiff.ps1` trigger is `-Weekly -DaysOfWeek Monday..Friday` at `07:30/12:30/16:30`, task name `Work Inbox Laptop Draft Diff`, `StartWhenAvailable` OFF, `wscript.exe`+hidden-VBS action.
- **`docs/LAPTOP_MIGRATION_PLAN.md` @ `main`** = Rev 2, 2026-08-29, unchanged, matches record. `~/.codex/config.toml` sha1 baseline `35f8910382373d525598194b2649159cfeed3f6a` (the DESKTOP file) still recorded in the doc; no `codex login`; Phase 2(ii) not started.
- **Bridge briefing** still runs from the laptop on cadence (`bced96b` 1 Sep 07:04 etc., 07:04/12:04/16:04). Per the 30 Aug bridge handover the desktop `Work Inbox Briefing` task is DISABLED -- not re-verified this dispatch.

## Discrepancy vs the 30 Aug "NEXT (gated)" list
Steps 1-3 (refresh / supervised run #2 / register) look done, but with no run-#2 report and no checkpoint, and the Mon-Fri-only trigger does not explain the Sunday 31 Aug runs (manual runs, or a modified trigger). **Kevin to reconcile:** on the LAPTOP `Get-ScheduledTask -TaskName 'Work Inbox Laptop Draft Diff'` (State + Triggers) + `Get-ScheduledTaskInfo` (LastRunTime/LastTaskResult/NextRunTime); on the DESKTOP `Get-ScheduledTask -TaskName 'Draft Diff Capture'` (State). Step 4 -- `Disable-ScheduledTask -TaskName 'Draft Diff Capture'` on the desktop -- is still the open gated action, to be done once the laptop task state is confirmed and one more clean run is seen.

## Track 2 (Lane B -- calendar + Teams) readiness
Today (1 Sep) is the plan's Lane B kickoff date. **Edu quota reset CONFIRMED (Kevin, 1 Sep)** -- "codex is reset for begb0037 (university)"; the quota-contention concern in plan sec.5a is now moot in practice, though the full-manifest auto-disable guard stays mandatory (sec.5e). Still pending, all Kevin's:
- (a) move interactive AI work off the Edu account (to personal Plus / Claude);
- (b) in ChatGPT Edu -> Connectors, remove the GitHub + Outlook Email connectors, leave only Outlook Calendar + Microsoft Teams, and confirm back exactly what remains;
- (c) on the laptop `codex login` to the Edu account, then the read-only `codex exec -s read-only --skip-git-repo-check` tool-enumeration proof (plan sec.7 "Deferred -- 1-Sept Lane B kickoff block") -- MAKE-OR-BREAK #2: headless `codex exec` must load exactly the {Calendar, Teams} read tools and nothing else (28 Aug desktop test loaded ZERO connector tools). Capture the LAPTOP's own `~/.codex/config.toml` sha1 before/after the enumeration -- they must be equal across the run; that file is distinct from the desktop `35f891...` baseline and is likely absent until `codex login`.

## Next action (one line)
Kevin reconciles the `Work Inbox Laptop Draft Diff` vs desktop `Draft Diff Capture` task states and reports; then either close Track 1 (gated `Disable-ScheduledTask 'Draft Diff Capture'` on the desktop) or start Track 2 -- Edu connector strip + laptop `codex login` + the read-only tool-enumeration proof.

---

# Handover -- 30 August 2026, ~evening UTC (Drew) -- DRAFT DIFF CAPTURE ported to laptop / IMAP + `claude -p` enrichment. Supervised laptop run PASSED (Thread-Index premise CONFIRMED). `com`/`api` path byte-identical, gated. Toast watcher covers draft-diff + FAILURE and no longer flashes a console window (wscript+hidden-VBS). Left: ONE more supervised run (now with enrichment + status push), then register + disable the desktop task.
...
| `docs/desktop-scripts/Watch-BridgeBriefing-Hidden.vbs` (NEW) + `Register-BridgeBriefingToastWatcher.ps1` (EDIT) | **Console-flash fix.** A direct `powershell.exe` task action flashes a window on every fire even with `-WindowStyle Hidden` -- and the toast watcher fires every 5 min, so Kevin had a popup on screen constantly. Fixed live by the coordinator (ad-hoc VBS + repointed action); now folded into the repo: the register script (re)generates `Watch-BridgeBriefing-Hidden.vbs` next to the live `.ps1` and registers `wscript.exe <vbs>` (WScript.Shell.Run windowStyle=0, synchronous, `WScript.Quit(rc)`). A redeploy can't reintroduce the flash. `Register-LaptopDraftDiff.ps1` got the same treatment (generates `Run Laptop Draft Diff-Hidden.vbs`, registers `wscript.exe`). Both parse-checked; the watcher VBS was run via `cscript` end-to-end (exit 0, poll line logged, no window). |

**STANDING RULE (add to any future frequently-firing scheduled task):** never register a direct `powershell.exe` action -- always `wscript.exe` + a hidden VBS wrapper (`WScript.Shell.Run(cmd, 0, True)` then `WScript.Quit(rc)`). Task Scheduler "Hidden" and `-WindowStyle Hidden` do NOT stop the initial console flash. **Still on direct `powershell.exe` (flag, not fixed this session -- bridge scope):** `Register-LaptopBridgeBriefing.ps1` (3x/day) and `Register-LaptopParityShadow.ps1` (6x/day) -- give them the same wscript+VBS wrapper next time they're touched.
