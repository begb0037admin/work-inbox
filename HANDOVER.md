...
Tested: green offline against the coordinator's real 31.5 KB probe (5 events, `08:30 UTC -> 09:30 BST` conversion, all-day heuristic, every `start` `datetime.fromisoformat()`-parseable); guard verb-classification unit-tested (`search_events`+`list_events` -> ok; `microsoft_outlook_email.send_message` / `github.*` / `create_event` / non-codex_apps -> halt); **and end-to-end on the real machine 1 Sept ~14:57** (`list_calendars`+`list_events`+`search_events` fired, real data -- the run that surfaced + then verified the guard fix).
