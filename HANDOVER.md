# Handover -- 30 August 2026, ~evening UTC (Drew) -- DRAFT DIFF CAPTURE ported to laptop / IMAP (code on `main`, behind unset `MAIL_BACKEND`). Desktop `Draft Diff Capture` NOT touched. Toast watcher extended to cover draft-diff + FAILURE. Needs ONE supervised laptop run before registering the recurring task.

## DRAFT DIFF CAPTURE -> laptop / IMAP (30 Aug, this entry)

**Why:** Kevin's call -- "the more we have migrated the better." `tools/draft_final_diff_capture.py` used a bare `Dispatch("Outlook.Application").GetNamespace("MAPI")` (no retry) and had been failing since the 28 Aug reboot (`AttributeError: Outlook.Application.GetNamespace`). Same move as the mail pull: read the mailbox over IMAP+OAuth2 from the laptop, no Outlook COM. (Note: COM actually recovered on the desktop with the M365 login fix -- a `--stats-only` run today returned `drafts_tracked_now: 104` clean -- but the migration proceeds regardless per Kevin.)

**Shipped on `main` (all default-OFF -- `MAIL_BACKEND` unset everywhere -> `com` path byte-identical):**

| File | What |
|---|---|
| `draft_diff_imap.py` (NEW, repo root) | IMAP backend for the diff capture, mirrors `imap_mail.py` (imports its `acquire_token_silent` / `_imap_connect` / helpers verbatim -- one auth impl, no drift). `snapshot_drafts_imap()` -> EXAMINE `\Drafts` ("Drafts"); `SentIndex` -> EXAMINE `\Sent` ("Sent Items"), header-only index + lazy body fetch. Read-only, `smtplib` never imported. |
| `tools/draft_final_diff_capture.py` (EDIT) | `MAIL_BACKEND` (env or `--mail-backend`), default `com`. `com` branch: `import win32com` + `snapshot_drafts`/`find_sent_match` **unchanged**. `imap` branch: never imports win32com. New `--stats-out PATH` writes the result dict (counts/paths only, no email content) for the wrapper's status push. `com` artifacts (`draft_final_diffs.json`, redaction log, backlog, failures json) byte-identical -- new `final_message_id` key is added **only when non-empty** (i.e. imap only); `confirmed_via` wording branches on `final_entry_id` presence. Only additive change on `com` is a `"mail_backend":"com"` key in the **stdout stats dict** (never written to any artifact file). |
| `docs/desktop-scripts/Run Laptop Draft Diff.ps1` (NEW) | Laptop wrapper. Refreshes the 5 scripts from main, `MAIL_BACKEND=imap`, `--no-ai` by default (see AI note), local-only staging `%USERPROFILE%\Documents\CorpusStaging\draft_watch_imap\` (its **own** ledger -- Thread-Index key scheme != COM ConversationID, so a first run re-baselines to ZERO pairs, by design). Then pushes the run-status file. `-WithAI` (requires `ANTHROPIC_API_KEY`), `-NoStatusPush`. |
| `docs/desktop-scripts/Register-LaptopDraftDiff.ps1` (NEW) | Task `Work Inbox Laptop Draft Diff`, `ad-oak\begb0037`, Interactive/Limited (run-only-when-logged-on), `IgnoreNew`, `PT20M`, **StartWhenAvailable OFF** (mirrors the desktop task -- skipped catch-up costs nothing). `-Cadence Bridge` (default) = **07:30 / 12:30 / 16:30 Mon-Fri** (30 min after each bridge-briefing slot); `-Cadence Full` = 06:30/09:30/12:30/15:30/18:30 (matches the desktop task exactly). |
| `docs/desktop-scripts/Push-LaptopRunStatus.ps1` (NEW) | Shared helper. PUTs `data/laptop_status/{briefing,draftdiff}_status.json` via the GitHub Contents API -- **counts / exit code / timestamp only, whitelisted keys, no email content ever**. Best-effort: any failure logs + exits 0, never fails a real run. |
| `docs/desktop-scripts/Watch-BridgeBriefing.ps1` (EDIT) | Now polls 3 things: (1) new `chore: update briefing` commit -> "updated" toast (unchanged); (2) `briefing_status.json` -> `result=failed` -> **"Work Inbox Briefing - FAILED (laptop)"** toast (success still comes from #1, no double-toast); (3) `draftdiff_status.json` -> `ok` -> "ran (vanished V / pairs P / backlog B)", `failed` -> **"Work Inbox Draft Diff - FAILED (laptop)"**. State file gains `lastBriefingStatusSha` / `lastDraftDiffSha`. 404 (file not yet created) = silent no-op. **Still a dumb poll+toast on purpose -- the consolidated cross-machine notification design is MARKEY scope, not this. Do not over-build here.** |
| `docs/desktop-scripts/Run Laptop Bridge Briefing.ps1` (EDIT) | Added `Publish-Status` -> calls `Push-LaptopRunStatus.ps1 -Kind briefing` at all 4 exit paths (best-effort, never changes the exit code). Gives the watcher the briefing FAILED signal.|

**AI classification note:** the script's `edit_type`/`note` step uses the Anthropic SDK directly and needs `ANTHROPIC_API_KEY`; the laptop deliberately has none (27 Aug cutover stopped the ~GBP 36/mo API charge). Wrapper runs `--no-ai` -> pairs are still **correlated + whole-pair-redacted** and staged in `pending_classification.json`; only the AI enrichment waits for a later keyed drain (or a future `claude -p` port -- a separate job, flagged for Kevin). The time-critical part (capturing the draft text before the draft vanishes) is fully preserved.

**Correlation-key fidelity gap (the one real risk):** IMAP has no `PR_CONVERSATION_ID`. `draft_diff_imap._conv_key()` derives an equivalent from the `Thread-Index` header (decoded bytes[6:22] = the conversation GUID Outlook itself hashes into ConversationID; Exchange stamps it on both the draft and the sent copy). Fallbacks if absent: thread-root Message-ID (`References`/`In-Reply-To`), then normalised `Thread-Topic`. **Must be verified on the supervised run** that drafts in the server-side Drafts folder actually carry `Thread-Index` over IMAP -- the run logs a `key scheme {...}` line for exactly this. If Thread-Index is missing on drafts, correlation degrades to the weaker fallbacks -> escalate before registering.

**Side effect from today's verification:** two `--stats-only` runs of the edited script on the desktop (COM path, to prove it's untouched) re-stamped the live desktop `ledger.json` `last_seen` values to 30 Aug 14:59 (pre-existing behaviour -- `save_ledger` runs unconditionally before the `stats_only` return). The still-active desktop `Draft Diff Capture` task (next run 31 Aug 06:30) now baselines against "drafts as of 30 Aug 14:59". No pairs newly lost -- that task has been failing since 28 Aug anyway.

### NEXT (gated -- Kevin runs these, one at a time; report the supervised-run result BEFORE registering)
1. **Copy scripts to the laptop run dir** `%USERPROFILE%\work-inbox\`: `Run Laptop Draft Diff.ps1`, `Push-LaptopRunStatus.ps1` (curl from `main` raw, same as the bridge scripts).
2. **ONE supervised run:** `cd $env:USERPROFILE\work-inbox ; .\"Run Laptop Draft Diff.ps1"` -- watch the console. Confirm: IMAP silent token OK (no browser), `Drafts mailbox 'Drafts': N item(s)`, a `key scheme {...}` line dominated by `TIDX:` (not `REF:`/`TOPIC:`), `Sent index: ... distinct conversation key(s)`, exit 0, `draftdiff_status.json` pushed. First run = 0 pairs by design (fresh imap ledger). Compare `drafts_tracked_now` to the desktop's 104.
3. **Report back**, then: `powershell -File .\Register-LaptopDraftDiff.ps1` (default cadence).
4. **Only once proven:** disable the desktop task -- `Disable-ScheduledTask -TaskName 'Draft Diff Capture'` (re-enable = `Enable-ScheduledTask -TaskName 'Draft Diff Capture'`).
