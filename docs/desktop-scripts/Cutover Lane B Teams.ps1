<#
Cutover Lane B Teams.ps1
========================
ONE-SHOT cutover of the live "Work Inbox Bridge Briefing" scheduled task from
TEAMS_BACKEND=off to TEAMS_BACKEND=connector (Lane B connector Teams digest),
on the personal ChatGPT account via the dedicated CODEX_HOME. Adds
-TeamsBackend connector to the task's existing args -- does NOT touch or
remove any existing -CalBackend flag already on the task.

Modelled directly on Cutover Lane B Calendar.ps1 (same structure, same
preconditions, same backup/rollback discipline) -- see that script's own
header for the general shape. This is the Teams-domain equivalent, written
2 Sept 2026 once PR #34 (the -TeamsBackend wrapper wiring) merged to main.

Kevin pastes this ONCE in his RDP session on 101L-DE013193, as AD-OAK\begb0037.
Expect it to take ~25-30 min end to end (a connector probe ~3-5 min, then a
full guarded briefing run ~15-20 min). It PRINTS a rollback one-liner and a
PASTE-THIS-BACK block at the end.

It changes NOTHING and exits non-zero if any precondition fails. It does NOT
touch the mail/IMAP path, and does NOT touch/remove any existing -CalBackend
flag. It is safe to re-run (idempotent-ish: timestamped backups, skips steps
already done).

USAGE (in the RDP PowerShell session, as AD-OAK\begb0037):
    cd $env:USERPROFILE\work-inbox
    $u='https://raw.githubusercontent.com/begb0037admin/work-inbox/main/docs/desktop-scripts/Cutover%20Lane%20B%20Teams.ps1'
    Invoke-WebRequest -UseBasicParsing "$u`?t=$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())" -OutFile '.\Cutover Lane B Teams.ps1'
    powershell -NoProfile -ExecutionPolicy Bypass -File '.\Cutover Lane B Teams.ps1'
...
$transcript = Join-Path $logdir "cutover_lane_b_teams_$ts.log"
$transcribing = $false
...
$R = [ordered]@{
  ...
  new_action = ''; guard_exit = ''; teams_line = ''; briefing_before = ''; briefing_after = ''; fetch_exit = ''
...
# --- probe: does the personal account have the Teams connector, and does the
#     re-contamination guard come back clean? ---
Say "probing the connector via lane_b_call1.py --domain teams (this makes ~1 real codex call, ~3-5 min on this box)..."
...
$env:PYTHONUTF8            = '1'
# NOTE: Tee-Object here writes to a SEPARATE file from $transcript -- Start-Transcript
# holds $transcript open exclusively; piping this subprocess's output into that same
# path fails every time on this host/PS combo (found + fixed during tonight's calendar
# cutover run, 2 Sept 2026).
& python -u (Join-Path $RunDir 'lane_b_call1.py') --domain teams 2>&1 | Tee-Object -FilePath ($transcript + '.probe.log') -Append
Say "lane_b_call1.py exit $LASTEXITCODE"

$norm = Join-Path $RunDir 'data\lane_b\lane_b_normalised.json'
if (-not (Test-Path $norm)) { Fail "probe produced no $norm -- connector unavailable this run. If it keeps happening, check the Microsoft Teams connector is attached to your PERSONAL ChatGPT account (Settings -> Connectors/Apps), then re-run." }
$doc = $null
try { $doc = Get-Content $norm -Raw | ConvertFrom-Json } catch { Fail "could not parse $norm : $($_.Exception.Message)" }
$teamsStatus = "$($doc.meta.lane_b.domains.teams.status)"
$teamsCount  = [int]$doc.meta.lane_b.domains.teams.count
Say "probe: teams status='$teamsStatus'  count=$teamsCount  tool_calls=$($doc.meta.lane_b.domains.teams.tool_calls -join ', ')"
$R.precond_probe = "status=$teamsStatus count=$teamsCount"
if ($teamsStatus -eq 'halt') { Fail "the re-contamination guard HALTED on the probe (a write tool appeared). DO NOT cut over. See data\codex_runs\GUARD_TRIPPED_*." }
if ($teamsStatus -ne 'ok') {
  Fail "probe did not come back clean (status='$teamsStatus', count=$teamsCount). Check the Microsoft Teams connector is attached to the personal account, then re-run. (count=0 alone is not fatal -- an empty chat window is plausible -- but status must be 'ok'.)"
}
Say "PRECONDITION: connector Teams probe OK -- $teamsCount item(s) on the personal account"
...
# ============================================================================
# 3. FLIP  ($LaneBCodexHome in the wrapper, pulled fresh from main -- same
#           override point used for both calendar and Teams; harmless if it's
#           already set from tonight's calendar cutover)
# ============================================================================
...
if ($wtxt -notmatch '\[ValidateSet\(''off'',''connector''\)\]\s*\[string\]\$TeamsBackend') {
  Fail "the fresh wrapper from main has no -TeamsBackend param -- PR #34 may not be on main yet. Do NOT proceed; ping Drew."
}
...
Say "wrote patched wrapper (with `\`$LaneBCodexHome = '$CodexHome', -TeamsBackend support, and the guard glue) -> $livePs1"

# ============================================================================
# 4. RE-REGISTER  (Set-ScheduledTask -Action -- preserves triggers/principal/
#    settings; ADDS -TeamsBackend connector, leaves any existing -CalBackend
#    flag untouched)
# ============================================================================
Say "--- 4. RE-REGISTER task action (+ -TeamsBackend connector) ---"
$newArgs = $args0
if ($newArgs -match '-TeamsBackend\s+connector') {
  Say "task action already has -TeamsBackend connector -- leaving args as-is (idempotent)"
}
elseif ($newArgs -match '-TeamsBackend\s+off') {
  $newArgs = $newArgs -replace '-TeamsBackend\s+off', '-TeamsBackend connector'
}
else {
  $newArgs = ($newArgs.TrimEnd() + ' -TeamsBackend connector')
}
...
try {
  ...
  Say "MANUAL FALLBACK: Register-ScheduledTask -Xml (Get-Content '$xmlBak' -Raw) -TaskName '$TaskName' -Force   (then edit the <Arguments> line to add ' -TeamsBackend connector')"
...
if ($after.Arguments -notmatch '-TeamsBackend\s+connector') { Fail "re-register did not take -- action still: $($after.Arguments)" }
...
if ($SkipRun) {
  ...
  if ($latestLog) {
    ...
    $c = $lt | Select-String 'Lane B connector Teams:' | Select-Object -Last 1
    $f = $lt | Select-String 'fetch_inbox\.py exit' | Select-Object -Last 1
    $R.guard_exit  = if ($g) { "$($g.Line.Trim())" } else { '(not found in log)' }
    $R.teams_line  = if ($c) { "$($c.Line.Trim())" } else { '(not found in log)' }
...
$R.rollback_b      = "`$a=(Get-ScheduledTask -TaskName '$TaskName').Actions[0]; Set-ScheduledTask -TaskName '$TaskName' -Action (New-ScheduledTaskAction -Execute `$a.Execute -Argument (`$a.Arguments -replace ' -TeamsBackend connector','') -WorkingDirectory `$a.WorkingDirectory)"
