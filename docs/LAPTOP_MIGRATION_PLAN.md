...
3. **Calendar-source degradation** — if a Lane B run can't reach the calendar connector (`codex exec` didn't load it that cycle), do Phases 3.7/3.8 degrade to *empty calendar + warning* and the briefing continues, or does the whole run fail? (Recommend: degrade + warn; the re-contamination guard already toasts.)
