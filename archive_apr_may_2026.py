...
2026 -- and anything before April 2026 -- must be left untouched. No other
folder is read or written. Nothing is ever deleted, only moved.
...
WINDOW_END = datetime(2026, 6, 1, 0, 0, 0)  # exclusive -- everything from June onward is excluded


def find_apr_may_items(inbox_folder):
    ...
    while item is not None:
        ...
        try:
            ...
            if received is not None and WINDOW_START <= received < WINDOW_END:
                matches.append({
                    "com_item": item,
                    "subject": subject,
                    "received": received,
...
def report(matches):
    if not matches:
        log("No April/May 2026 items found in Inbox. Nothing to archive.")
        return
    by_month = {}
    for m in matches:
        key = m["received"].strftime("%Y-%m")
        by_month.setdefault(key, []).append(m)
    log(f"{len(matches)} Inbox items dated April/May 2026 match the archive window:")
    ...
    log(f"Date range of matched items: {oldest.strftime('%Y-%m-%d')} to {newest.strftime('%Y-%m-%d')}")


def write_report_json(matches, path):
    payload = {
...
def main():
    ...
    matches = find_apr_may_items(inbox_folder)
    report(matches)

    out_name = "archive_apr_may_2026_executed.json" if execute else "archive_apr_may_2026_dryrun.json"
    write_report_json(matches, os.path.join(os.path.dirname(os.path.abspath(__file__)), out_name))
    ...
    remaining = find_apr_may_items(inbox_folder)
    if remaining:
        log(f"WARNING: {len(remaining)} April/May 2026 items still present in Inbox after the move.")
    else:
        log("Verified: 0 April/May 2026 items remain in Inbox.")
